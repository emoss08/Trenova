...
func (c *LocationCategory) BeforeUpdate(_ context.Context) error {
	c.Version++

	return nil
}

func (c *LocationCategory) Insert(ctx context.Context, tx bun.IDB, auditService *audit.Service, user audit.AuditUser) error {
	...
	auditService.LogAction(
		constants.TableLocationCategory,
		c.ID.String(),
...
func (c *LocationCategory) UpdateOne(ctx context.Context, tx bun.IDB, auditService *audit.Service, user audit.AuditUser) error {
	original := new(LocationCategory)
...
func (c *LocationCategory) OptimisticUpdate(ctx context.Context, tx bun.IDB) error {
	ov := c.Version
	...
	if rows == 0 {
		return &validator.BusinessLogicError{
			Message: fmt.Sprintf("Version mismatch. The LocationCategory (ID: %s) has been updated by another user. Please refresh and try again.", c.ID),
		}
	}
