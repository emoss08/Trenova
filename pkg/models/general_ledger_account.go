...
func (g GeneralLedgerAccount) Validate() error {
	return validation.ValidateStruct(
		...
		validation.Field(&g.AccountNumber,
			validation.Required,
			validation.Length(7, 7).Error("Account number must be 7 characters. Please try again"),
			validation.Match(regexp.MustCompile("^[0-9]{4}-[0-9]{2}$")).Error("Account number must be in the format ####-##. Please try again"),
		),
