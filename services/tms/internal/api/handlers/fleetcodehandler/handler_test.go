...
func TestFleetCodeHandler_List_WithPagination(t *testing.T) {
	...
	items := make([]*fleetcode.FleetCode, 0, requestedLimit+1)
	for i := range requestedLimit + 1 {
		items = append(items, &fleetcode.FleetCode{
			ID:             pulid.MustNew("fc_"),
			OrganizationID: testutil.TestOrgID,
			BusinessUnitID: testutil.TestBuID,
			Code:           fmt.Sprintf("FC%02d", i),
			Status:         domaintypes.StatusActive,
	...
	repo.On("List", mock.Anything, mock.MatchedBy(
		func(req *repositories.ListFleetCodesRequest) bool {
...
func TestFleetCodeHandler_List_RejectsOffset(t *testing.T) {
	t.Parallel()

	handler := setupFleetCodeHandler(t, mocks.NewMockFleetCodeRepository(t))
