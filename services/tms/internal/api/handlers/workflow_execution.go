...
func (h *WorkflowExecutionHandler) RegisterRoutes(rg *gin.RouterGroup) {
	...
	api.GET("", h.pm.RequirePermission(permission.ResourceWorkflowExecution, "read"), h.list)
	api.GET(":id/", h.pm.RequirePermission(permission.ResourceWorkflowExecution, "read"), h.get)
	api.GET(
		":id/steps/",
		h.pm.RequirePermission(permission.ResourceWorkflowExecution, "read"),
		h.getSteps,
	)
	api.POST(
		":id/cancel/",
		h.pm.RequirePermission(permission.ResourceWorkflowExecution, "update"),
		h.cancel,
	)
	api.POST(
		":id/retry/",
		h.pm.RequirePermission(permission.ResourceWorkflowExecution, "create"),
		h.retry,
	)
...
func (h *WorkflowExecutionHandler) trigger(c *gin.Context) {
	...
	execution, err := h.service.TriggerWorkflow(
		c.Request.Context(),
		&workflowservice.TriggerWorkflowRequest{
			WorkflowID:  workflowID,
			OrgID:       authCtx.OrganizationID,
			BuID:        authCtx.BusinessUnitID,
			UserID:      authCtx.UserID,
...
func (h *WorkflowExecutionHandler) retry(c *gin.Context) {
	...
	execution, err := h.service.RetryExecution(
		c.Request.Context(),
		id,
		authCtx.OrganizationID,
		authCtx.BusinessUnitID,
