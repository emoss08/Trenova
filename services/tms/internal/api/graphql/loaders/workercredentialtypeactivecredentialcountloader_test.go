...
func TestWorkerCredentialTypeActiveCredentialCountBatchFunc_MapsCountsAndDefaultsMissingToZero(
	t *testing.T,
) {
	t.Parallel()
	...
	firstID := pulid.MustNew("wcrt_")
	secondID := pulid.MustNew("wcrt_")
	counter := stubTenantCounter{
		count: func(_ context.Context, ti pagination.TenantInfo, ids []pulid.ID) (map[pulid.ID]int, error) {
			...
			return map[pulid.ID]int{firstID: 9}, nil
		},
	}
	factory := &WorkerCredentialTypeActiveCredentialCountLoaderFactory{counter: counter}
	...
	assert.Equal(t, 9, values[3])
}

func TestWorkerCredentialTypeActiveCredentialCountBatchFunc_RepositoryErrorFillsValidResults(
	t *testing.T,
) {
	t.Parallel()

	typeID := pulid.MustNew("wcrt_")
