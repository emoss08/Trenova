...
func TestPerformanceReviewTemplateOpenReviewCountBatchFunc_MapsCountsAndDefaultsMissingToZero(
	t *testing.T,
) {
	t.Parallel()
	...
	firstID := pulid.MustNew("prt_")
	secondID := pulid.MustNew("prt_")
	counter := stubTenantCounter{
		count: func(_ context.Context, ti pagination.TenantInfo, ids []pulid.ID) (map[pulid.ID]int, error) {
			...
			return map[pulid.ID]int{firstID: 6}, nil
		},
	}
	factory := &PerformanceReviewTemplateOpenReviewCountLoaderFactory{counter: counter}
	...
	assert.Equal(t, 6, values[0])
	require.Error(t, errs[1])
	require.NoError(t, errs[2])
	assert.Zero(t, values[2])
	require.NoError(t, errs[3])
	assert.Equal(t, 6, values[3])
}

func TestPerformanceReviewTemplateOpenReviewCountBatchFunc_RepositoryErrorFillsValidResults(
	t *testing.T,
) {
	t.Parallel()

	templateID := pulid.MustNew("prt_")
