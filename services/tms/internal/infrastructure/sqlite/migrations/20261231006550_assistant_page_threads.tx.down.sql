-- Hand-written: the reverse of the up migration. Draft edits go, the page
-- conversations go with their messages, and assistant_artifacts is rebuilt
-- with the kind check it had before.
-- Source: 20261231006550_assistant_page_threads.tx.down.sql

DELETE FROM "assistant_artifacts" WHERE "kind" = 'draft_edit';
...
DELETE FROM "assistant_artifacts" WHERE "thread_id" IN (
    SELECT "id" FROM "assistant_threads" WHERE "origin" IN ('Import', 'Formula')
);
...
DELETE FROM "assistant_turns" WHERE "thread_id" IN (
    SELECT "id" FROM "assistant_threads" WHERE "origin" IN ('Import', 'Formula')
);

--bun:split

DELETE FROM "assistant_threads" WHERE "origin" IN ('Import', 'Formula');
