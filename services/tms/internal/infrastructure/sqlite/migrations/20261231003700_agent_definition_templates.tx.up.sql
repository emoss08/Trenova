...
-- is rebuilt with the widened template check and its rows copied across.
-- agent_tool_trust references it with ON DELETE CASCADE, and dropping the old
-- table would delete every trust row through that cascade, so the trust rows
-- are set aside first and the table is recreated after the rebuild.
-- Source: 20261231003700_agent_definition_templates.tx.up.sql

CREATE TEMP TABLE "agent_tool_trust_backup" AS SELECT * FROM "agent_tool_trust";

--bun:split

DROP TABLE "agent_tool_trust";
...
INSERT INTO "agent_definitions_rebuilt" (
    ...
    "instructions", "guardrails", "tool_names", "tool_tiers", "autonomy_ceiling", "enabled",
    "shadow_mode", "decision_timeout_seconds", "trigger_mode", "cron_expression", "cron_timezone",
    "event_kinds", "interval_seconds", "ends_at", "max_concurrent_runs", "run_timeout_seconds",
    "max_tool_calls", "context_providers", "output_mode", "preferred_provider_id", "system_key",
    "last_run_at", "next_run_at", "version", "created_at", "updated_at", "icon", "accent",
    "monthly_budget_usd", "daily_run_limit", "tool_daily_limits", "simulation_mode"
)
SELECT
    "id", "business_unit_id", "organization_id", "name", "description", "template",
    "instructions", "guardrails", "tool_names", "tool_tiers", "autonomy_ceiling", "enabled",
...
CREATE UNIQUE INDEX "uq_agent_tool_trust_agent_tool"
