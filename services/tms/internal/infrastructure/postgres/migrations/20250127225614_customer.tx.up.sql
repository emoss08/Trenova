...
--bun:split
CREATE OR REPLACE FUNCTION customers_geom_sync()
    RETURNS TRIGGER
    AS $$
BEGIN
...
--bun:split
DROP TRIGGER IF EXISTS customers_geom_sync_trigger ON customers;

--bun:split
CREATE TRIGGER customers_geom_sync_trigger
    BEFORE INSERT OR UPDATE ON customers
    FOR EACH ROW
    EXECUTE FUNCTION customers_geom_sync();
