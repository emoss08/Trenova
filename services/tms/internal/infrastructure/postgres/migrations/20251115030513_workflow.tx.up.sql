...
CREATE TABLE IF NOT EXISTS "workflows"(
    ...
    CONSTRAINT fk_workflows_organization FOREIGN KEY("organization_id", "business_unit_id") REFERENCES organizations(id, business_unit_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "workflow_templates"(
    ...
    PRIMARY KEY ("id", "organization_id", "business_unit_id"),
    CONSTRAINT fk_workflow_templates_organization FOREIGN KEY("organization_id", "business_unit_id") REFERENCES organizations(id, business_unit_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE INDEX idx_workflow_versions_published ON workflow_versions(is_published) WHERE is_published = true;
...
CREATE INDEX idx_workflow_executions_temporal ON workflow_executions(temporal_workflow_id) WHERE temporal_workflow_id IS NOT NULL;
CREATE INDEX idx_workflow_executions_created_at ON workflow_executions(created_at DESC);
CREATE INDEX idx_workflow_executions_completed_at ON workflow_executions(completed_at DESC) WHERE completed_at IS NOT NULL;
...
CREATE INDEX idx_workflow_templates_system ON workflow_templates(is_system_template) WHERE is_system_template = true;
