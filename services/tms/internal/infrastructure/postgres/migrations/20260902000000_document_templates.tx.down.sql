-- Rolling this back must leave the schema exactly as the previous migration group
-- produced it, which means recreating the two dormant tables and five generic enums
-- this migration dropped. That is verbose, but a rollback that leaves them missing
-- would make a re-run of the forward migration fail on the DROPs and would break
-- migrator.Reset.
ALTER TABLE "detention_notices"
    DROP CONSTRAINT IF EXISTS "fk_detention_notices_pdf_document";

--bun:split
ALTER TABLE "detention_notices"
    DROP CONSTRAINT IF EXISTS "fk_detention_notices_template_version";
...
ALTER TABLE "document_templates"
    DROP CONSTRAINT IF EXISTS "fk_document_templates_active_version";

--bun:split
DROP TABLE IF EXISTS "generated_documents";

--bun:split
DROP TABLE IF EXISTS "document_template_assignments";
...
--bun:split
DROP TYPE IF EXISTS "generated_document_delivery_enum";

--bun:split
DROP TYPE IF EXISTS "generated_document_status_enum";

--bun:split
DROP TYPE IF EXISTS "document_template_orientation_enum";

--bun:split
DROP TYPE IF EXISTS "document_template_page_size_enum";

--bun:split
DROP TYPE IF EXISTS "document_template_version_status_enum";

--bun:split
-- Everything below restores 20251125000001_add_document_templates verbatim.
...
--bun:split
ALTER TABLE "document_templates"
    ADD COLUMN IF NOT EXISTS search_vector tsvector GENERATED ALWAYS AS (setweight(immutable_to_tsvector('english', COALESCE("code", '')), 'A') || setweight(immutable_to_tsvector('english', COALESCE("name", '')), 'A') || setweight(immutable_to_tsvector('english', COALESCE("description", '')), 'B')) STORED;
...
ALTER TABLE "generated_documents"
    ADD COLUMN IF NOT EXISTS search_vector tsvector GENERATED ALWAYS AS (setweight(immutable_to_tsvector('english', COALESCE("file_name", '')), 'A') || setweight(immutable_to_tsvector('english', COALESCE("reference_type", '')), 'B')) STORED;
