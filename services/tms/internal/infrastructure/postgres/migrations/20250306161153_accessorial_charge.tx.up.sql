...
-- Search Vector and Relationship Indexes SQL for AccessorialCharge
...
CREATE INDEX IF NOT EXISTS idx_accessorial_charges_search_vector ON "accessorial_charges" USING GIN(search_vector);

--bun:split
-- 4. Search Vector Trigger Function
CREATE OR REPLACE FUNCTION accessorial_charges_search_trigger()
    RETURNS TRIGGER
    AS $$
BEGIN
...
DROP TRIGGER IF EXISTS accessorial_charges_search_update ON "accessorial_charges";

CREATE TRIGGER accessorial_charges_search_update
    BEFORE INSERT OR UPDATE ON "accessorial_charges"
    FOR EACH ROW
    EXECUTE FUNCTION accessorial_charges_search_trigger();
