...
var shipmentCopyPlan = map[string]fieldPlan{
	...
	"BillToCustomerID":  {copiedFromSource, "a copy bills the same payer"},
	"FreightTerms":      {copiedFromSource, "who pays the carrier carries over"},
	"ChargeAllocations": {setByCopy, "the payer split is rebuilt against the copied charges"},
	"PreviousCustomerID": {
		intentionallyDropped,
		"transient: only an update carries the customer it replaced",
	},
	...
	"BillToCustomer":  {intentionallyDropped, "relation, hydrated on read"},
