...
func NewRepository(p Params) repositories.EquipmentTypeRepository {
	return &repository{
		db: p.DB,
		l:  p.Logger.Named("postgres.equipmenttype-repository"),
	}
}
...
func (r *repository) List(
	...
	entities := make([]*equipmenttype.EquipmentType, 0, req.Filter.Limit)
...
func (r *repository) GetByID(
	...
	err = db.NewSelect().Model(entity).
		WhereGroup(" AND ", func(sq *bun.SelectQuery) *bun.SelectQuery {
			return sq.
				Where("et.id = ?", req.ID).
				Where("et.organization_id = ?", req.OrgID).
				Where("et.business_unit_id = ?", req.BuID)
		}).Scan(ctx)
	if err != nil {
		return nil, dberror.HandleNotFoundError(err, "Equipment Type")
	}
...
func (r *repository) Create(
	...
	if _, err = db.NewInsert().Model(entity).Returning("*").Exec(ctx); err != nil {
		log.Error("failed to insert equipment type", zap.Error(err))
		return nil, err
	}
...
func (r *repository) Update(
	...
	results, rErr := db.NewUpdate().
		...
		Exec(ctx)
	if rErr != nil {
		log.Error("failed to update equipment type", zap.Error(rErr))
		return nil, rErr
	}

	roErr := dberror.CheckRowsAffected(results, "Equipment Type", entity.ID.String())
	if roErr != nil {
		return nil, roErr
	}
