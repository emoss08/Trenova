...
func TestAccountsReceivableRepositoryReturnsLedgerAndAging(t *testing.T) {
	...
	ledgerProjectionRepo := customerledgerrepository.New(
	...
	control.DefaultUnappliedCashAccountID = lookupARGLAccount(
		t,
		ctx,
		db,
		org.ID,
	...
	inv, err := invoiceRepo.Create(
		...
		&invoice.Invoice{
			...
			DueDate:            int64PtrAR(period.EndDate),
			...
			[]*customerledger.Entry{
				{
					...
					SourceObjectID:   inv.ID.String(),
					SourceEventType:  tenant.JournalSourceEventInvoicePosted.String(),
	...
	paymentSvc := customerpaymentservice.New(
		customerpaymentservice.Params{
			...
			CustomerLedgerRepo: ledgerProjectionRepo,
	...
	ledger, err := repo.ListCustomerLedger(
		ctx,
		repositories.ListCustomerLedgerRequest{
	...
	aging, err := repo.ListARAging(
		ctx,
		repositories.ListARAgingRequest{
			TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID},
			AsOfDate:   now.Unix(),
		},
	)
...
func TestAccountsReceivableRepositoryReturnsOpenItems(t *testing.T) {
	...
	_, err = paymentSvc.PostAndApply(
		...
		&serviceports.PostCustomerPaymentRequest{
			...
			AmountMinor:     5000,
			PaymentMethod:   customerpayment.MethodACH,
			ReferenceNumber: "PAY-AR-OPEN",
			CurrencyCode:    "USD",
			Applications: []*serviceports.CustomerPaymentApplicationInput{
				{InvoiceID: inv.ID, AppliedAmountMinor: 5000},
			},
			TenantInfo: pagination.TenantInfo{
				OrgID:  org.ID,
				BuID:   org.BusinessUnitID,
...
func TestAccountsReceivableRepositoryReturnsCustomerNameAndAging(t *testing.T) {
	...
	_, err = paymentSvc.PostAndApply(
		...
		&serviceports.PostCustomerPaymentRequest{
			...
			AmountMinor:     2500,
			PaymentMethod:   customerpayment.MethodACH,
			ReferenceNumber: "PAY-AR-STMT",
			CurrencyCode:    "USD",
			Applications: []*serviceports.CustomerPaymentApplicationInput{
				{InvoiceID: inv.ID, AppliedAmountMinor: 2500},
			},
			TenantInfo: pagination.TenantInfo{
				OrgID:  org.ID,
				BuID:   org.BusinessUnitID,
	...
	name, err := repo.GetCustomerName(
		ctx,
		repositories.GetARCustomerNameRequest{
			TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID},
			CustomerID: shp.CustomerID,
		},
	)
	...
	aging, err := repo.GetCustomerAging(
		ctx,
