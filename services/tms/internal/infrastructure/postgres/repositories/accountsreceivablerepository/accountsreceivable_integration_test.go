...
func TestAccountsReceivableRepositoryReturnsOpenItems(t *testing.T) {
	ctx, db, cleanup := seedtest.SetupTestDB(t)
	defer cleanup()
	seedRegistry := seeder.NewRegistry()
	seeds.Register(seedRegistry)
	...
	queue := &billingqueue.BillingQueueItem{OrganizationID: org.ID, BusinessUnitID: org.BusinessUnitID, ShipmentID: shp.ID, Number: "INV-AR-OPEN-1", Status: billingqueue.StatusPosted, BillType: billingqueue.BillTypeInvoice}
	_, err = db.NewInsert().Model(queue).Exec(ctx)
	require.NoError(t, err)
	postedAt := now.Unix()
	inv, err := invoiceRepo.Create(ctx, &invoice.Invoice{OrganizationID: org.ID, BusinessUnitID: org.BusinessUnitID, BillingQueueItemID: queue.ID, ShipmentID: shp.ID, CustomerID: shp.CustomerID, Number: queue.Number, BillType: billingqueue.BillTypeInvoice, Status: invoice.StatusPosted, PostedAt: &postedAt, PaymentTerm: invoice.PaymentTermNet30, CurrencyCode: "USD", InvoiceDate: period.StartDate, DueDate: int64PtrAR(period.EndDate), ShipmentProNumber: shp.ProNumber, ShipmentBOL: shp.BOL, BillToName: "Test Customer", SubtotalAmount: decimal.NewFromInt(100), OtherAmount: decimal.Zero, TotalAmount: decimal.NewFromInt(100), AppliedAmount: decimal.Zero, SettlementStatus: invoice.SettlementStatusUnpaid, DisputeStatus: invoice.DisputeStatusNone, Lines: []*invoice.Line{{LineNumber: 1, Type: invoice.LineTypeFreight, Description: "Freight", Quantity: decimal.NewFromInt(1), UnitPrice: decimal.NewFromInt(100), Amount: decimal.NewFromInt(100)}}})
	...
	_, err = paymentSvc.PostAndApply(ctx, &serviceports.PostCustomerPaymentRequest{CustomerID: shp.CustomerID, PaymentDate: now.Unix(), AccountingDate: now.Unix(), AmountMinor: 5000, PaymentMethod: customerpayment.MethodACH, ReferenceNumber: "PAY-AR-OPEN", CurrencyCode: "USD", Applications: []*serviceports.CustomerPaymentApplicationInput{{InvoiceID: inv.ID, AppliedAmountMinor: 5000}}, TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID, UserID: user.ID}}, testutil.NewSessionActor(user.ID, org.ID, org.BusinessUnitID))
	require.NoError(t, err)

	items, err := repo.ListOpenItems(ctx, repositories.ListAROpenItemsRequest{TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID}, CustomerID: shp.CustomerID, AsOfDate: now.Unix()})
	require.NoError(t, err)
	require.Len(t, items, 1)
	assert.Equal(t, inv.ID, items[0].InvoiceID)
	assert.Equal(t, int64(10000), items[0].TotalAmountMinor)
	assert.Equal(t, int64(5000), items[0].AppliedAmountMinor)
	assert.Equal(t, int64(5000), items[0].OpenAmountMinor)
	assert.Equal(t, shp.CustomerID, items[0].CustomerID)
}

func TestAccountsReceivableRepositoryReturnsCustomerNameAndAging(t *testing.T) {
	ctx, db, cleanup := seedtest.SetupTestDB(t)
	defer cleanup()
	seedRegistry := seeder.NewRegistry()
	seeds.Register(seedRegistry)
	...
	queue := &billingqueue.BillingQueueItem{OrganizationID: org.ID, BusinessUnitID: org.BusinessUnitID, ShipmentID: shp.ID, Number: "INV-AR-STMT-1", Status: billingqueue.StatusPosted, BillType: billingqueue.BillTypeInvoice}
	_, err = db.NewInsert().Model(queue).Exec(ctx)
	require.NoError(t, err)
	postedAt := now.Unix()
	inv, err := invoiceRepo.Create(ctx, &invoice.Invoice{OrganizationID: org.ID, BusinessUnitID: org.BusinessUnitID, BillingQueueItemID: queue.ID, ShipmentID: shp.ID, CustomerID: shp.CustomerID, Number: queue.Number, BillType: billingqueue.BillTypeInvoice, Status: invoice.StatusPosted, PostedAt: &postedAt, PaymentTerm: invoice.PaymentTermNet30, CurrencyCode: "USD", InvoiceDate: period.StartDate, DueDate: int64PtrAR(period.EndDate), ShipmentProNumber: shp.ProNumber, ShipmentBOL: shp.BOL, BillToName: "Test Customer", SubtotalAmount: decimal.NewFromInt(100), OtherAmount: decimal.Zero, TotalAmount: decimal.NewFromInt(100), AppliedAmount: decimal.Zero, SettlementStatus: invoice.SettlementStatusUnpaid, DisputeStatus: invoice.DisputeStatusNone, Lines: []*invoice.Line{{LineNumber: 1, Type: invoice.LineTypeFreight, Description: "Freight", Quantity: decimal.NewFromInt(1), UnitPrice: decimal.NewFromInt(100), Amount: decimal.NewFromInt(100)}}})
	...
	paymentSvc := customerpaymentservice.New(customerpaymentservice.Params{Logger: logger, DB: conn, Repo: paymentRepo, InvoiceRepo: invoiceRepo, AccountingRepo: accountingRepo, JournalRepo: journalRepo, Generator: generator, Validator: customerpaymentservice.NewValidator(customerpaymentservice.ValidatorParams{InvoiceRepo: invoiceRepo, FiscalPeriodRepo: fiscalPeriodRepo}), AuditService: &mocks.NoopAuditService{}})
	_, err = paymentSvc.PostAndApply(ctx, &serviceports.PostCustomerPaymentRequest{CustomerID: shp.CustomerID, PaymentDate: now.Unix(), AccountingDate: now.Unix(), AmountMinor: 2500, PaymentMethod: customerpayment.MethodACH, ReferenceNumber: "PAY-AR-STMT", CurrencyCode: "USD", Applications: []*serviceports.CustomerPaymentApplicationInput{{InvoiceID: inv.ID, AppliedAmountMinor: 2500}}, TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID, UserID: user.ID}}, testutil.NewSessionActor(user.ID, org.ID, org.BusinessUnitID))
	require.NoError(t, err)

	name, err := repo.GetCustomerName(ctx, repositories.GetARCustomerNameRequest{TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID}, CustomerID: shp.CustomerID})
	require.NoError(t, err)
	require.NotEmpty(t, name)

	aging, err := repo.GetCustomerAging(ctx, repositories.GetARCustomerAgingRequest{TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID}, CustomerID: shp.CustomerID, AsOfDate: now.Unix()})
	require.NoError(t, err)
	require.NotNil(t, aging)
	assert.Equal(t, shp.CustomerID, aging.CustomerID)
	assert.Equal(t, int64(7500), aging.Buckets.TotalOpenMinor)
}
