...
func TestPolicyFor(t *testing.T) {
	...
	assert.True(
		t,
		missing.ShadowMode,
		"without a dispatch agent the organization's shadow setting applies",
	)
	...
	assert.False(
		t,
		disabled.DefinitionID.IsNil(),
		"runs still link to the definition that governs them",
	)
	...
	assert.True(
		t,
		shadowed.ShadowMode,
		"a definition in shadow mode stays in shadow even when the organization is live",
	)
