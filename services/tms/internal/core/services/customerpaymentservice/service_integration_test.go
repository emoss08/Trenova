...
func TestPostAndApplyCreatesPaymentApplicationJournalAndSettlementUpdate(t *testing.T) {
	...
	control.DefaultRevenueAccountID = lookupPaymentGLAccount(
		t,
		ctx,
		db,
		org.ID,
	...
	svc := New(
		Params{
			...
			Validator: NewValidator(
				ValidatorParams{InvoiceRepo: invoiceRepo, FiscalPeriodRepo: fiscalPeriodRepo},
			),
			AuditService: &mocks.NoopAuditService{},
		},
	)
	payment, err := svc.PostAndApply(
		ctx,
	...
	updatedInvoice, err := invoiceRepo.GetByID(
		ctx,
	...
	applicationCount, err = db.NewSelect().
		Table("customer_payment_applications").
	...
	require.NoError(
		...
		db.NewSelect().
			...
			Column("reference_type", "total_debit", "total_credit").
			Where("reference_id = ?", payment.ID.String()).
			Limit(1).
			Scan(ctx, &entry),
	)
	...
	var sourceCount int
	sourceCount, err = db.NewSelect().
		Table("journal_sources").
		Where("source_object_id = ?", payment.ID.String()).
		Count(ctx)
	require.NoError(t, err)
...
func TestPostAndApplySupportsUnappliedCash(t *testing.T) {
	...
	lineCount, err = db.NewSelect().
		...
		Where("journal_entry_id = (SELECT id FROM journal_entries WHERE reference_id = ? LIMIT 1)", payment.ID.String()).
		Count(ctx)
	require.NoError(t, err)
	...
	require.NoError(
		...
		db.NewSelect().
			...
			Column("period_credit_minor").
			Where("gl_account_id = ?", control.DefaultUnappliedCashAccountID).
			Where("fiscal_period_id = ?", period.ID).
			Limit(1).
			Scan(ctx, &unappliedBalance),
	)
...
func TestApplyUnappliedLaterCreatesReclassificationEntry(t *testing.T) {
	...
	require.NoError(t, err)
	postedAt := now.Unix()
	inv1, err := invoiceRepo.Create(
		ctx,
		&invoice.Invoice{
			OrganizationID:     org.ID,
			BusinessUnitID:     org.BusinessUnitID,
			BillingQueueItemID: queue1.ID,
			ShipmentID:         shp.ID,
			CustomerID:         shp.CustomerID,
	...
	require.NoError(t, err)
	inv2, err := invoiceRepo.Create(
		ctx,
		&invoice.Invoice{
			OrganizationID:     org.ID,
			BusinessUnitID:     org.BusinessUnitID,
			BillingQueueItemID: queue2.ID,
			ShipmentID:         shp.ID,
			CustomerID:         shp.CustomerID,
			Number:             queue2.Number,
			BillType:           billingqueue.BillTypeInvoice,
			Status:             invoice.StatusPosted,
			PostedAt:           &postedAt,
			PaymentTerm:        invoice.PaymentTermNet30,
			...
			ShipmentBOL:        shp.BOL,
			BillToName:         "Test Customer",
			SubtotalAmount:     decimal.NewFromInt(50),
			OtherAmount:        decimal.Zero,
	...
	payment, err := svc.PostAndApply(
		...
		&serviceports.PostCustomerPaymentRequest{
			...
			CurrencyCode:    "USD",
			Applications: []*serviceports.CustomerPaymentApplicationInput{
				{InvoiceID: inv1.ID, AppliedAmountMinor: 10000},
			},
			TenantInfo: pagination.TenantInfo{
				OrgID:  org.ID,
				BuID:   org.BusinessUnitID,
	...
	payment, err = svc.ApplyUnapplied(
		ctx,
		&serviceports.ApplyCustomerPaymentRequest{
			PaymentID:      payment.ID,
			AccountingDate: now.Unix(),
			Applications: []*serviceports.CustomerPaymentApplicationInput{
				{InvoiceID: inv2.ID, AppliedAmountMinor: 5000},
			},
			TenantInfo: pagination.TenantInfo{
				OrgID:  org.ID,
				BuID:   org.BusinessUnitID,
	...
	updatedInvoice2, err := invoiceRepo.GetByID(
		ctx,
		repositories.GetInvoiceByIDRequest{
			ID:         inv2.ID,
			TenantInfo: pagination.TenantInfo{OrgID: org.ID, BuID: org.BusinessUnitID},
	...
	appliedEntryCount, err = db.NewSelect().
		Table("journal_entries").
		Where("reference_type = ?", "CustomerPaymentApplied").
		Where("reference_id = ?", payment.ID.String()).
		Count(ctx)
	require.NoError(t, err)
...
func TestReversePaymentRestoresInvoiceAndBalances(t *testing.T) {
	...
	reversalEntryCount, err = db.NewSelect().
		Table("journal_entries").
		Where("reference_type = ?", tenant.JournalSourceEventCustomerPaymentReversed.String()).
		Where("reference_id = ?", payment.ID.String()).
		Count(ctx)
	require.NoError(t, err)
	...
	require.NoError(
		...
		db.NewSelect().
			Table("gl_account_balances_by_period").
			Column("net_change_minor").
			Where("gl_account_id = ?", control.DefaultARAccountID).
			Where("fiscal_period_id = ?", period.ID).
			Limit(1).
			Scan(ctx, &arBalance),
	)
	...
	require.NoError(
		...
		db.NewSelect().
			Table("gl_account_balances_by_period").
			Column("net_change_minor").
			Where("gl_account_id = ?", control.DefaultCashAccountID).
			Where("fiscal_period_id = ?", period.ID).
			Limit(1).
...
func TestPostAndApplyRecognizesShortPayAndSettlesInvoice(t *testing.T) {
	...
	control.DefaultWriteOffAccountID = lookupPaymentGLAccount(
		t,
		ctx,
		db,
		org.ID,
	...
	payment, err := svc.PostAndApply(
		...
		&serviceports.PostCustomerPaymentRequest{
			...
			AmountMinor:     9000,
			PaymentMethod:   customerpayment.MethodACH,
			ReferenceNumber: "PAY-SP",
			CurrencyCode:    "USD",
			Applications: []*serviceports.CustomerPaymentApplicationInput{
				{InvoiceID: inv.ID, AppliedAmountMinor: 9000, ShortPayAmountMinor: 1000},
			},
			TenantInfo: pagination.TenantInfo{
				OrgID:  org.ID,
				BuID:   org.BusinessUnitID,
	...
	require.NoError(
		...
		db.NewSelect().
			Table("gl_account_balances_by_period").
			Column("period_debit_minor").
			Where("gl_account_id = ?", control.DefaultWriteOffAccountID).
			Where("fiscal_period_id = ?", period.ID).
			Limit(1).
			Scan(ctx, &writeOffBalance),
	)
	assert.Equal(t, int64(1000), writeOffBalance.PeriodDebitMinor)
}

func TestListAndGetCustomerPayments(t *testing.T) {
	ctx, db, cleanup := seedtest.SetupTestDB(t)
	defer cleanup()
	seedRegistry := seeder.NewRegistry()
	seeds.Register(seedRegistry)
	...
	listResult, err := svc.List(
		ctx,
		&repositories.ListCustomerPaymentsRequest{
