...
func (f *fakeRepo) ListRecognitions(
	_ context.Context,
...
func (f *fakeRepo) CreateRecognition(
	_ context.Context,
	r *worker.WorkerRecognition,
) (*worker.WorkerRecognition, error) {
...
func TestCreateEvent_DefaultsExpiryAndScrubsKindSpecificFields(t *testing.T) {
	...
	created, err := h.svc.CreateEvent(
		context.Background(),
		h.event(worker.SafetyEventAccident, func(e *worker.WorkerSafetyEvent) {
			e.Points = 4
			e.Preventable = true
	...
	_, err = h.svc.CreateEvent(
		context.Background(),
		h.event(worker.SafetyEventInspection, nil),
		h.userID,
	)
...
func TestCloseAndDeleteEvent(t *testing.T) {
	...
	created, err := h.svc.CreateEvent(
		context.Background(),
		h.event(worker.SafetyEventIncident, nil),
		h.userID,
	)
	require.NoError(t, err)
	...
	reopened, err := h.svc.ReopenEvent(
		context.Background(),
		&workersafetyservice.EventStatusRequest{
			ID: created.ID, TenantInfo: h.tenant, UserID: h.userID,
		},
	)
	require.NoError(t, err)
...
func TestIssueAction_ClimbsTheLadderAndRecordsEmployment(t *testing.T) {
	...
	days := int32(3)
	suspension, err := h.svc.IssueAction(
		context.Background(),
		&workersafetyservice.IssueActionRequest{
			TenantInfo: h.tenant, WorkerID: h.wrk.ID, Level: worker.DisciplinaryLevelSuspension,
	...
	rescinded, err := h.svc.RescindAction(
		context.Background(),
		&workersafetyservice.ActionStatusRequest{
	...
	acknowledged, err := h.svc.AcknowledgeAction(
		context.Background(),
		h.tenant,
