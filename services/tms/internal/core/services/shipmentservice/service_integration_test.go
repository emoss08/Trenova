...
func TestCreateIntegration_PersistsAdditionalChargesAndCommodities(t *testing.T) {
	...
	svc, shipmentRepo, controlRepo, _, tenantInfo, fixture, data := newIntegrationShipmentService(t, ctx, db)
	...
	commodityEntity := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "General Freight", pulid.Nil)
	...
	created, err := svc.Create(ctx, entity, internaltestutil.NewSessionActor(data.User.ID, tenantInfo.OrgID, tenantInfo.BuID))
...
func TestUpdateIntegration_SyncsNestedChargesAndCommodities(t *testing.T) {
	...
	updated, err := svc.Update(ctx, loaded, internaltestutil.NewSessionActor(data.User.ID, tenantInfo.OrgID, tenantInfo.BuID))
	require.NoError(t, err)
...
func TestUpdateIntegration_ReplacesExistingCommodityInPlace(t *testing.T) {
	...
	commodityOne := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "Replacement One", pulid.Nil)
	commodityTwo := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "Replacement Two", pulid.Nil)
...
func TestCreateIntegration_RejectsHazmatSegregationConflicts(t *testing.T) {
	...
	leftCommodity := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "Explosives", leftHazmat.ID)
...
func TestCreateIntegration_RatesWithHydratedCommodityDetails(t *testing.T) {
	...
	commodityOne := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "Rate Commodity One", hazmat.ID)
	commodityTwo := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "Rate Commodity Two", pulid.Nil)
...
func TestUpdateIntegration_RecalculatesWithHydratedCommodityDetails(t *testing.T) {
	...
	commodityOne := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "Rate Update One", pulid.Nil)
	commodityTwo := testutil.MustCreateCommodity(t, ctx, db, tenantInfo, "Rate Update Two", pulid.Nil)
	...
	_, err = db.NewUpdate().
		...
		Where("id IN (?)", bun.In([]pulid.ID{commodityOne.ID, commodityTwo.ID})).
		Exec(ctx)
	require.NoError(t, err)
...
func newIntegrationShipmentService(
	...
	*seedtest.TestData,
	...
	additionalChargeRepo := shipmentadditionalchargerepository.New(shipmentadditionalchargerepository.Params{
		DB:     conn,
		Logger: zap.NewNop(),
	})
...
func makeIntegrationShipment(
	...
	origin := testutil.MustCreateLocation(t, ctx, db, fixture, tenantInfo, pulid.MustNew("loc_").String(), "Origin")
	destination := testutil.MustCreateLocation(t, ctx, db, fixture, tenantInfo, pulid.MustNew("loc_").String(), "Destination")
