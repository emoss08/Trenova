	...
	return validationframework.
		...
		WithValidation(func(
			...
			if _, ok := registry.Get(entity.Kind); !ok {
				multiErr.Add(fieldKind, errortypes.ErrInvalid, "\"{0}\" is not a template kind this system can render", entity.Kind)
			}
...
func (v *Validator) ValidateVersion(
	...
	if !def.Paged && (entity.PageSize != "" || entity.Orientation != "") {
		channelErr.Add("pageSize", errortypes.ErrInvalid, "{0} is a message, not a document, so it has no page setup", def.DisplayName)
	}
...
func (v *Validator) checkChannelsBelong(
	...
	if entity.Subject != "" &&
		...
		multiErr.Add("subject", errortypes.ErrInvalid, "{0} does not have a subject line", def.DisplayName)
	}

	if entity.BodyText != "" &&
		!def.HasChannel(documenttemplate.ChannelEmailText) &&
		!def.HasChannel(documenttemplate.ChannelNotificationBody) {
		multiErr.Add("bodyText", errortypes.ErrInvalid, "{0} does not have a plain-text body", def.DisplayName)
	}
	...
	if (entity.HeaderHTML != "" || entity.FooterHTML != "") &&
		...
		multiErr.Add("headerHtml", errortypes.ErrInvalid, "{0} is not printed, so it has no running header or footer", def.DisplayName)
	}
}
...
func (v *Validator) ValidateAssignment(
	...
	switch {
	case !ok:
		scopeErr.Add("templateId", errortypes.ErrInvalid, "\"{0}\" is not a template kind this system can render", template.Kind)
	case !def.CustomerScoped:
		...
		scopeErr.Add(fieldKind, errortypes.ErrInvalid, "{0} is not sent per customer, so assigning it to one has no effect", def.DisplayName)
	}
