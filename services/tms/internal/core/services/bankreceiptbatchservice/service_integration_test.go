...
func TestImportBatchPersistsMixedOutcomesAndSupportsReads(t *testing.T) {
	...
	result, err := env.batchService.Import(env.ctx, &serviceports.ImportBankReceiptBatchRequest{
		...
		Receipts: []*serviceports.ImportBankReceiptBatchLine{
			{ReceiptDate: env.now.Unix(), AmountMinor: 10000, ReferenceNumber: "BANK-MATCH-1", Memo: "auto-match"},
			{ReceiptDate: env.now.Add(time.Hour).Unix(), AmountMinor: 5000, ReferenceNumber: "NO-MATCH-1", Memo: "exception"},
		},
		TenantInfo: pagination.TenantInfo{OrgID: env.org.ID, BuID: env.org.BusinessUnitID, UserID: env.user.ID},
	}, actor)
	...
	detail, err := env.batchService.Get(env.ctx, &serviceports.GetBankReceiptBatchRequest{BatchID: result.Batch.ID, TenantInfo: pagination.TenantInfo{OrgID: env.org.ID, BuID: env.org.BusinessUnitID, UserID: env.user.ID}})
	require.NoError(t, err)
	...
	batches, err := env.batchService.List(env.ctx, pagination.TenantInfo{OrgID: env.org.ID, BuID: env.org.BusinessUnitID, UserID: env.user.ID})
	require.NoError(t, err)
	require.NotEmpty(t, batches)
	assert.Equal(t, result.Batch.ID, batches[0].ID)

	count, err := env.db.NewSelect().Table("bank_receipts").Where("import_batch_id = ?", result.Batch.ID).Where("organization_id = ?", env.org.ID).Where("business_unit_id = ?", env.org.BusinessUnitID).Count(env.ctx)
	require.NoError(t, err)
...
func TestImportBatchRollsBackOnNilReceiptLine(t *testing.T) {
	...
	result, err := env.batchService.Import(env.ctx, &serviceports.ImportBankReceiptBatchRequest{
		...
		Receipts: []*serviceports.ImportBankReceiptBatchLine{
			{ReceiptDate: env.now.Unix(), AmountMinor: 10000, ReferenceNumber: "ROLLBACK-MATCH-1", Memo: "should rollback"},
			nil,
		},
		TenantInfo: pagination.TenantInfo{OrgID: env.org.ID, BuID: env.org.BusinessUnitID, UserID: env.user.ID},
	}, actor)
	...
	batchCount, err := env.db.NewSelect().Table("bank_receipt_import_batches").Where("reference = ?", "BATCH-ROLLBACK-1").Where("organization_id = ?", env.org.ID).Where("business_unit_id = ?", env.org.BusinessUnitID).Count(env.ctx)
	require.NoError(t, err)
	assert.Equal(t, 0, batchCount)

	receiptCount, err := env.db.NewSelect().Table("bank_receipts").Where("reference_number = ?", "ROLLBACK-MATCH-1").Where("organization_id = ?", env.org.ID).Where("business_unit_id = ?", env.org.BusinessUnitID).Count(env.ctx)
	require.NoError(t, err)
...
func setupBatchIntegrationEnv(t *testing.T) *batchIntegrationEnv {
	...
	batchRepo := bankreceiptbatchrepository.New(bankreceiptbatchrepository.Params{DB: conn, Logger: logger})
	workItemRepo := bankreceiptworkitemrepository.New(bankreceiptworkitemrepository.Params{DB: conn, Logger: logger})
	...
	require.NoError(t, db.NewSelect().Table("shipments").Column("id", "customer_id", "pro_number", "bol").Where("organization_id = ?", org.ID).Where("business_unit_id = ?", org.BusinessUnitID).Limit(1).Scan(ctx, &shipment))

	control, err := accountingRepo.GetByOrgID(ctx, org.ID)
	require.NoError(t, err)
	control.DefaultCashAccountID = lookupBatchGLAccount(t, ctx, db, org.ID, org.BusinessUnitID, "1010")
	control.DefaultUnappliedCashAccountID = lookupBatchGLAccount(t, ctx, db, org.ID, org.BusinessUnitID, "2200")
	control.DefaultARAccountID = lookupBatchGLAccount(t, ctx, db, org.ID, org.BusinessUnitID, "1110")
	control.ReconciliationMode = tenant.ReconciliationModeWarnOnly
	_, err = accountingRepo.Update(ctx, control)
	require.NoError(t, err)

	receiptSvc := bankreceiptservice.New(bankreceiptservice.Params{Logger: logger, Repo: receiptRepo, WorkItemRepo: workItemRepo, PaymentRepo: paymentRepo, AccountingRepo: accountingRepo, AuditService: &mocks.NoopAuditService{}})
	batchSvc := New(Params{Logger: logger, DB: conn, Repo: batchRepo, ReceiptRepo: receiptRepo, BankReceiptService: receiptSvc, AuditService: &mocks.NoopAuditService{}})

	return &batchIntegrationEnv{ctx: ctx, db: db, batchService: batchSvc, org: org, user: user, shipment: shipment, now: time.Now().UTC(), cleanup: cleanup}
}

func createPostedCustomerPayment(t *testing.T, env *batchIntegrationEnv, referenceNumber string, amountMinor int64) {
	t.Helper()
	...
	now := env.now
	fy, err := fiscalYearRepo.Create(env.ctx, &fiscalyear.FiscalYear{OrganizationID: env.org.ID, BusinessUnitID: env.org.BusinessUnitID, Status: fiscalyear.StatusOpen, Year: now.Year(), Name: "FY", StartDate: now.Add(-24 * time.Hour).Unix(), EndDate: now.Add(24 * time.Hour).Unix(), IsCurrent: true, AllowAdjustingEntries: true})
	require.NoError(t, err)
	period, err := fiscalPeriodRepo.Create(env.ctx, &fiscalperiod.FiscalPeriod{OrganizationID: env.org.ID, BusinessUnitID: env.org.BusinessUnitID, FiscalYearID: fy.ID, PeriodNumber: 1, PeriodType: fiscalperiod.PeriodTypeMonth, Status: fiscalperiod.StatusOpen, Name: "Period", StartDate: now.Add(-24 * time.Hour).Unix(), EndDate: now.Add(24 * time.Hour).Unix(), AllowAdjustingEntries: true})
	require.NoError(t, err)

	queue := &billingqueue.BillingQueueItem{OrganizationID: env.org.ID, BusinessUnitID: env.org.BusinessUnitID, ShipmentID: env.shipment.ID, Number: "INV-" + referenceNumber, Status: billingqueue.StatusPosted, BillType: billingqueue.BillTypeInvoice}
	_, err = env.db.NewInsert().Model(queue).Exec(env.ctx)
	require.NoError(t, err)
	postedAt := now.Unix()
	inv, err := invoiceRepo.Create(env.ctx, &invoice.Invoice{OrganizationID: env.org.ID, BusinessUnitID: env.org.BusinessUnitID, BillingQueueItemID: queue.ID, ShipmentID: env.shipment.ID, CustomerID: env.shipment.CustomerID, Number: queue.Number, BillType: billingqueue.BillTypeInvoice, Status: invoice.StatusPosted, PostedAt: &postedAt, PaymentTerm: invoice.PaymentTermNet30, CurrencyCode: "USD", InvoiceDate: period.StartDate, DueDate: int64Ptr(period.EndDate), ShipmentProNumber: env.shipment.ProNumber, ShipmentBOL: env.shipment.BOL, BillToName: "Test Customer", SubtotalAmount: decimal.NewFromInt(amountMinor / 100), OtherAmount: decimal.Zero, TotalAmount: decimal.NewFromInt(amountMinor / 100), AppliedAmount: decimal.Zero, SettlementStatus: invoice.SettlementStatusUnpaid, DisputeStatus: invoice.DisputeStatusNone, Lines: []*invoice.Line{{LineNumber: 1, Type: invoice.LineTypeFreight, Description: "Freight", Quantity: decimal.NewFromInt(1), UnitPrice: decimal.NewFromInt(amountMinor / 100), Amount: decimal.NewFromInt(amountMinor / 100)}}})
	require.NoError(t, err)
	...
	_, err = paymentSvc.PostAndApply(env.ctx, &serviceports.PostCustomerPaymentRequest{CustomerID: env.shipment.CustomerID, PaymentDate: now.Unix(), AccountingDate: now.Unix(), AmountMinor: amountMinor, PaymentMethod: customerpayment.MethodACH, ReferenceNumber: referenceNumber, CurrencyCode: "USD", Applications: []*serviceports.CustomerPaymentApplicationInput{{InvoiceID: inv.ID, AppliedAmountMinor: amountMinor}}, TenantInfo: pagination.TenantInfo{OrgID: env.org.ID, BuID: env.org.BusinessUnitID, UserID: env.user.ID}}, testutil.NewSessionActor(env.user.ID, env.org.ID, env.org.BusinessUnitID))
	require.NoError(t, err)
}

func lookupBatchGLAccount(t *testing.T, ctx context.Context, db *bun.DB, orgID, buID pulid.ID, accountCode string) pulid.ID {
	t.Helper()
