...
func (e *emitter) checkOwnScopeSupport() error {
	for entityKey, scope := range e.az.scopes {
		...
		if entity.OwnershipColumn == "" {
			return errortypes.NewAuthorizationError("your access to {0} is limited to your own records, but {1} does not support per-user scoping in reports", entity.PluralLabel, entity.PluralLabel)
		}
	}
	return nil
}
