...
// GrantRelationChain provides a fluent API for building nested relationship chains
type GrantRelationChain struct {
	relations []string
	options   map[string]func(*bun.SelectQuery) *bun.SelectQuery
}

// NewGrantRelationChain creates a new relation chain builder
func NewGrantRelationChain() *GrantRelationChain {
	return &GrantRelationChain{
		relations: []string{},
...
func (rc *GrantRelationChain) Add(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *GrantRelationChain {
...
func (rc *GrantRelationChain) Build() []string {
	return rc.relations
}

// Apply applies the relation chain to a query
func (rc *GrantRelationChain) Apply(q *bun.SelectQuery) *bun.SelectQuery {
...
func (b *GrantQueryBuilder) WithUser() *GrantRelationChainBuilder {
	chain := &GrantRelationChainBuilder{
		parent: b,
		chain:  NewGrantRelationChain(),
	}
...
func (b *GrantQueryBuilder) WithPermission() *GrantRelationChainBuilder {
	chain := &GrantRelationChainBuilder{
		parent: b,
		chain:  NewGrantRelationChain(),
	}
...
// WithGrantor creates a relation chain starting with Grantor
func (b *GrantQueryBuilder) WithGrantor() *GrantRelationChainBuilder {
	chain := &GrantRelationChainBuilder{
		parent: b,
		chain:  NewGrantRelationChain(),
	}
	chain.chain.Add("Grantor")
	return chain
}

// WithRevoker creates a relation chain starting with Revoker
func (b *GrantQueryBuilder) WithRevoker() *GrantRelationChainBuilder {
	chain := &GrantRelationChainBuilder{
		parent: b,
		chain:  NewGrantRelationChain(),
	}
	chain.chain.Add("Revoker")
	return chain
}

// GrantRelationChainBuilder provides fluent API for building nested relations
type GrantRelationChainBuilder struct {
	parent *GrantQueryBuilder
	chain  *GrantRelationChain
}

// Load applies the relation chain and returns to the parent builder
func (rb *GrantRelationChainBuilder) Load() *GrantQueryBuilder {
	rb.parent.query = rb.chain.Apply(rb.parent.query)
	return rb.parent
}

// ThenLoad adds another relation to the chain
func (rb *GrantRelationChainBuilder) ThenLoad(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *GrantRelationChainBuilder {
	rb.chain.Add(relation, opts...)
	return rb
}

// OrderBy adds ordering to the current relation in the chain
func (rb *GrantRelationChainBuilder) OrderBy(order string) *GrantRelationChainBuilder {
...
func (rb *GrantRelationChainBuilder) Where(condition string, args ...interface{}) *GrantRelationChainBuilder {
	if len(rb.chain.relations) > 0 {
		lastRel := rb.chain.relations[len(rb.chain.relations)-1]
		currentOpt := rb.chain.options[lastRel]
		rb.chain.options[lastRel] = func(q *bun.SelectQuery) *bun.SelectQuery {
