...
// EquipmentTypeRelationChain provides a fluent API for building nested relationship chains
type EquipmentTypeRelationChain struct {
	relations []string
	options   map[string]func(*bun.SelectQuery) *bun.SelectQuery
}

// NewEquipmentTypeRelationChain creates a new relation chain builder
func NewEquipmentTypeRelationChain() *EquipmentTypeRelationChain {
	return &EquipmentTypeRelationChain{
		relations: []string{},
...
func (rc *EquipmentTypeRelationChain) Add(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *EquipmentTypeRelationChain {
...
func (rc *EquipmentTypeRelationChain) Build() []string {
	return rc.relations
}

// Apply applies the relation chain to a query
func (rc *EquipmentTypeRelationChain) Apply(q *bun.SelectQuery) *bun.SelectQuery {
...
func (b *EquipmentTypeQueryBuilder) WithBusinessUnit() *EquipmentTypeRelationChainBuilder {
	chain := &EquipmentTypeRelationChainBuilder{
		parent: b,
		chain:  NewEquipmentTypeRelationChain(),
	}
...
func (b *EquipmentTypeQueryBuilder) WithOrganization() *EquipmentTypeRelationChainBuilder {
	chain := &EquipmentTypeRelationChainBuilder{
		parent: b,
		chain:  NewEquipmentTypeRelationChain(),
	}
...
// EquipmentTypeRelationChainBuilder provides fluent API for building nested relations
type EquipmentTypeRelationChainBuilder struct {
	parent *EquipmentTypeQueryBuilder
	chain  *EquipmentTypeRelationChain
}

// Load applies the relation chain and returns to the parent builder
func (rb *EquipmentTypeRelationChainBuilder) Load() *EquipmentTypeQueryBuilder {
	rb.parent.query = rb.chain.Apply(rb.parent.query)
	return rb.parent
}

// ThenLoad adds another relation to the chain
func (rb *EquipmentTypeRelationChainBuilder) ThenLoad(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *EquipmentTypeRelationChainBuilder {
	rb.chain.Add(relation, opts...)
	return rb
}

// OrderBy adds ordering to the current relation in the chain
func (rb *EquipmentTypeRelationChainBuilder) OrderBy(order string) *EquipmentTypeRelationChainBuilder {
...
func (rb *EquipmentTypeRelationChainBuilder) Where(condition string, args ...interface{}) *EquipmentTypeRelationChainBuilder {
	if len(rb.chain.relations) > 0 {
		lastRel := rb.chain.relations[len(rb.chain.relations)-1]
		currentOpt := rb.chain.options[lastRel]
		rb.chain.options[lastRel] = func(q *bun.SelectQuery) *bun.SelectQuery {
