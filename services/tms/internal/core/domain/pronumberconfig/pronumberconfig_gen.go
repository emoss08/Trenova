...
// ProNumberConfigRelationChain provides a fluent API for building nested relationship chains
type ProNumberConfigRelationChain struct {
	relations []string
	options   map[string]func(*bun.SelectQuery) *bun.SelectQuery
}

// NewProNumberConfigRelationChain creates a new relation chain builder
func NewProNumberConfigRelationChain() *ProNumberConfigRelationChain {
	return &ProNumberConfigRelationChain{
		relations: []string{},
...
func (rc *ProNumberConfigRelationChain) Add(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *ProNumberConfigRelationChain {
...
func (rc *ProNumberConfigRelationChain) Build() []string {
	return rc.relations
}

// Apply applies the relation chain to a query
func (rc *ProNumberConfigRelationChain) Apply(q *bun.SelectQuery) *bun.SelectQuery {
...
func (b *ProNumberConfigQueryBuilder) WithBusinessUnit() *ProNumberConfigRelationChainBuilder {
	chain := &ProNumberConfigRelationChainBuilder{
		parent: b,
		chain:  NewProNumberConfigRelationChain(),
	}
...
func (b *ProNumberConfigQueryBuilder) WithOrganization() *ProNumberConfigRelationChainBuilder {
	chain := &ProNumberConfigRelationChainBuilder{
		parent: b,
		chain:  NewProNumberConfigRelationChain(),
	}
...
// ProNumberConfigRelationChainBuilder provides fluent API for building nested relations
type ProNumberConfigRelationChainBuilder struct {
	parent *ProNumberConfigQueryBuilder
	chain  *ProNumberConfigRelationChain
}

// Load applies the relation chain and returns to the parent builder
func (rb *ProNumberConfigRelationChainBuilder) Load() *ProNumberConfigQueryBuilder {
	rb.parent.query = rb.chain.Apply(rb.parent.query)
	return rb.parent
}

// ThenLoad adds another relation to the chain
func (rb *ProNumberConfigRelationChainBuilder) ThenLoad(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *ProNumberConfigRelationChainBuilder {
	rb.chain.Add(relation, opts...)
	return rb
}

// OrderBy adds ordering to the current relation in the chain
func (rb *ProNumberConfigRelationChainBuilder) OrderBy(order string) *ProNumberConfigRelationChainBuilder {
...
func (rb *ProNumberConfigRelationChainBuilder) Where(condition string, args ...interface{}) *ProNumberConfigRelationChainBuilder {
	if len(rb.chain.relations) > 0 {
		lastRel := rb.chain.relations[len(rb.chain.relations)-1]
		currentOpt := rb.chain.options[lastRel]
		rb.chain.options[lastRel] = func(q *bun.SelectQuery) *bun.SelectQuery {
