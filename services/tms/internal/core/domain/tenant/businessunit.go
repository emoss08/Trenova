...
var (
	_ bun.BeforeAppendModelHook = (*BusinessUnit)(nil)
	_ domain.Validatable        = (*BusinessUnit)(nil)
)
...
type BusinessUnit struct {
	...
	Organizations      []*Organization  `json:"organizations"      bun:"rel:has-many,join:id=business_unit_id"`
}
...
func (bu *BusinessUnit) Validate(multiErr *errortypes.MultiError) {
	err := validation.ValidateStruct(bu,
		...
		validation.Field(
			...
			validation.When(
				bu.PrimaryEmail != "",
				validation.Required.Error("Email must not be empty if provided"),
			),
...
func (bu *BusinessUnit) GetTableName() string {
