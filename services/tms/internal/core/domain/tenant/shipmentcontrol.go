...
var (
	...
	_ validationframework.TenantedEntity = (*ShipmentControl)(nil)
)
...
type ShipmentControl struct {
	...
	AutoCancelShipments          bool      `json:"autoCancelShipments"          bun:"auto_cancel_shipments,type:BOOLEAN,notnull,default:false"`
	TrackDetentionTime           bool      `json:"trackDetentionTime"           bun:"track_detention_time,type:BOOLEAN,notnull,default:false"`
	AutoGenerateDetentionCharges bool      `json:"autoGenerateDetentionCharges" bun:"auto_generate_detention_charges,type:BOOLEAN,notnull,default:false"`
	DetentionChargeID            *pulid.ID `json:"detentionChargeId"            bun:"detention_charge_id,type:VARCHAR(100),nullzero"`
	UseDetentionPolicyEngine     bool      `json:"useDetentionPolicyEngine"     bun:"use_detention_policy_engine,type:BOOLEAN,notnull,default:false"`
...
func (sc *ShipmentControl) Validate(multiErr *errortypes.MultiError) {
	err := validation.ValidateStruct(
		...
		validation.Field(&sc.DetentionThreshold,
			validation.When(sc.TrackDetentionTime && !sc.UseDetentionPolicyEngine,
		...
		validation.Field(&sc.DefaultDetentionPolicyID,
			validation.When(
				sc.UseDetentionPolicyEngine,
				validation.Required.Error(
					"Select the organization default detention policy before enabling the policy engine",
				),
			),
		),
		...
		validation.Field(
			...
			validation.When(
				sc.TrackDetentionTime && !sc.UseDetentionPolicyEngine,
				validation.Required.Error(
