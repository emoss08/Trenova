...
// RatingBasis says what a rule's rate is multiplied by.
//
// Every basis but Matrix and Formula is a single multiplication, which is the
// point: a pricing analyst should never have to write an expression to say
// "two dollars fifteen a mile". Formula is the escape hatch for the shapes no
// list of bases will ever cover.
type RatingBasis string

const (
	RatingBasisFlat          = RatingBasis("Flat")
	RatingBasisPerMile       = RatingBasis("PerMile")
	RatingBasisPerCwt        = RatingBasis("PerCwt")
	RatingBasisPerPiece      = RatingBasis("PerPiece")
	RatingBasisPerStop       = RatingBasis("PerStop")
	RatingBasisPerPallet     = RatingBasis("PerPallet")
	RatingBasisPerLinearFoot = RatingBasis("PerLinearFoot")
	RatingBasisPerHour       = RatingBasis("PerHour")
	RatingBasisPercent       = RatingBasis("Percent")
	RatingBasisMatrix        = RatingBasis("Matrix")
	RatingBasisFormula       = RatingBasis("Formula")
)

func (rb RatingBasis) String() string {
	return string(rb)
}

func (rb RatingBasis) IsValid() bool {
	switch rb {
	case RatingBasisFlat,
		RatingBasisPerMile,
		RatingBasisPerCwt,
		RatingBasisPerPiece,
		RatingBasisPerStop,
...
// SupportsWeightBreaks reports whether a rule on this basis may carry banded
// rates. Only quantity based charges can be broken by weight.
func (rb RatingBasis) SupportsWeightBreaks() bool {
	return rb == RatingBasisPerCwt || rb == RatingBasisFlat
}

// PercentBasis says what a percentage rule takes its percentage of. It exists
// mostly for buy side agreements written as a share of the sell rate.
...
const (
	...
	PercentBasisSellRate                 = PercentBasis("SellRate")
)

func (pb PercentBasis) String() string {
	return string(pb)
}

func (pb PercentBasis) IsValid() bool {
	switch pb {
	case PercentBasisLinehaul,
