...
// ResourceDefinitionRelationChain provides a fluent API for building nested relationship chains
type ResourceDefinitionRelationChain struct {
	relations []string
	options   map[string]func(*bun.SelectQuery) *bun.SelectQuery
}

// NewResourceDefinitionRelationChain creates a new relation chain builder
func NewResourceDefinitionRelationChain() *ResourceDefinitionRelationChain {
	return &ResourceDefinitionRelationChain{
		relations: []string{},
...
func (rc *ResourceDefinitionRelationChain) Add(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *ResourceDefinitionRelationChain {
...
func (rc *ResourceDefinitionRelationChain) Build() []string {
	return rc.relations
}

// Apply applies the relation chain to a query
func (rc *ResourceDefinitionRelationChain) Apply(q *bun.SelectQuery) *bun.SelectQuery {
...
// ResourceDefinitionRelationChainBuilder provides fluent API for building nested relations
type ResourceDefinitionRelationChainBuilder struct {
	parent *ResourceDefinitionQueryBuilder
	chain  *ResourceDefinitionRelationChain
}

// Load applies the relation chain and returns to the parent builder
func (rb *ResourceDefinitionRelationChainBuilder) Load() *ResourceDefinitionQueryBuilder {
	rb.parent.query = rb.chain.Apply(rb.parent.query)
	return rb.parent
}

// ThenLoad adds another relation to the chain
func (rb *ResourceDefinitionRelationChainBuilder) ThenLoad(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *ResourceDefinitionRelationChainBuilder {
	rb.chain.Add(relation, opts...)
	return rb
}

// OrderBy adds ordering to the current relation in the chain
func (rb *ResourceDefinitionRelationChainBuilder) OrderBy(order string) *ResourceDefinitionRelationChainBuilder {
...
func (rb *ResourceDefinitionRelationChainBuilder) Where(condition string, args ...interface{}) *ResourceDefinitionRelationChainBuilder {
	if len(rb.chain.relations) > 0 {
		lastRel := rb.chain.relations[len(rb.chain.relations)-1]
		currentOpt := rb.chain.options[lastRel]
		rb.chain.options[lastRel] = func(q *bun.SelectQuery) *bun.SelectQuery {
