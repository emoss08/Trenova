...
func NewRegistry(p RegistryParams) *Registry {
	return &Registry{
		...
		logger:     p.Logger.Named("email-worker-registry"),
...
func (r *Registry) RegisterActivities(w worker.Worker) error {
	w.RegisterActivity(r.activities.RenderTemplateActivity)
	w.RegisterActivity(r.activities.SendEmailActivity)

	r.logger.Info("registered email activities",
		zap.Int("count", 2),
	)
