...
// DomainConfig shares the billing task queue. WorkerConfig must stay identical
// to the other registries on that queue — the manager reuses one worker per
// queue and rejects a registration whose options differ.
var DomainConfig = registry.DomainConfig{
	Name:         "billing-transfer-worker",
