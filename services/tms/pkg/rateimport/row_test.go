...
func TestParseRow_ReadsALaneAndItsPrice(t *testing.T) {
	...
	rule, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "PerMile", "2.35", "450"})
	...
	assert.Equal(t, rateagreement.RatingBasisPerMile, rule.RatingBasis)
...
func TestParseRow_BuildsTheLaneKeyTheEngineMatchesOn(t *testing.T) {
	...
	rule, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "PerMile", "2.35", ""})
...
func TestParseRow_HonoursAScopeColumnWhenTheSheetHasOne(t *testing.T) {
	...
	rule, err := rateimport.ParseRow(mapping, []string{"Zip3", "606", "State", "GA", "2.35"})
...
func TestParseRow_ABlankRowIsMarkedBlankRatherThanFailed(t *testing.T) {
	...
	rule, err := rateimport.ParseRow(mapping, []string{"", "", "", "", ""})
...
func TestParseRow_ARowWithNoPriceIsRefused(t *testing.T) {
	...
	_, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "PerMile", "", ""})
...
func TestParseRow_ARowWithHalfALaneIsRefused(t *testing.T) {
	...
	_, err := rateimport.ParseRow(mapping, []string{"IL", "", "PerMile", "2.35", ""})
...
func TestParseRow_ReadsMoneyTheWayASpreadsheetWritesIt(t *testing.T) {
	...
	rule, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "Flat", "$2,350.00", ""})
...
func TestParseRow_ARateThatIsNotANumberIsRefused(t *testing.T) {
	...
	_, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "Flat", "call for pricing", ""})
...
func TestParseRow_ReadsTheBasisTheWayAPersonWritesIt(t *testing.T) {
	...
	for _, spelling := range []string{"per mile", "PER_MILE", "Per-Mile", "permile"} {
		rule, err := rateimport.ParseRow(mapping, []string{"IL", "GA", spelling, "2.35", ""})

		require.NoErrorf(t, err, "spelling %q", spelling)
		assert.Equalf(t, rateagreement.RatingBasisPerMile, rule.RatingBasis, "spelling %q", spelling)
	}
}
...
func TestParseRow_AnUnrecognisedBasisIsRefused(t *testing.T) {
	...
	_, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "per furlong", "2.35", ""})
...
func TestParseRow_DefaultsToFlatWhenTheSheetHasNoBasisColumn(t *testing.T) {
	...
	rule, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "1500"})

	require.NoError(t, err)
	assert.Equal(t, rateagreement.RatingBasisFlat, rule.RatingBasis)
}
...
func TestParseRow_AShortRowDoesNotPanic(t *testing.T) {
	...
	rule, err := rateimport.ParseRow(mapping, []string{"IL", "GA", "Flat", "1500"})
