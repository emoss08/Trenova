...
type Validator struct {
	factory *framework.TenantedValidatorFactory[*servicetype.ServiceType]
}

func NewValidator(p ValidatorParams) *Validator {
	factory := framework.NewTenantedValidatorFactory[*servicetype.ServiceType](
		func(ctx context.Context) (*bun.DB, error) {
			return p.DB.DB(ctx)
		},
	).
		WithModelName("ServiceType").
		WithUniqueFields(func(entity *servicetype.ServiceType) []framework.UniqueField {
			return []framework.UniqueField{
				{
					Name:     "code",
					GetValue: func() string { return entity.Code },
					Message:  "Service type with code ':value' already exists in the organization.",
				},
			}
		}).
		WithCustomRules(func(entity *servicetype.ServiceType, vc *validator.ValidationContext) []framework.ValidationRule {
