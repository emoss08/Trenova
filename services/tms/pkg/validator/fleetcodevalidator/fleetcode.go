...
type Validator struct {
	factory *framework.TenantedValidatorFactory[*fleetcode.FleetCode]
}

func NewValidator(p ValidatorParams) *Validator {
	factory := framework.NewTenantedValidatorFactory[*fleetcode.FleetCode](
		func(ctx context.Context) (*bun.DB, error) {
			return p.DB.DB(ctx)
		},
	).
		WithModelName("FleetCode").
		WithUniqueFields(func(entity *fleetcode.FleetCode) []framework.UniqueField {
			return []framework.UniqueField{
				{
					Name:     "code",
					GetValue: func() string { return entity.Code },
					Message:  "Fleet code with code ':value' already exists in the organization.",
				},
			}
		}).
		WithCustomRules(func(entity *fleetcode.FleetCode, vc *validator.ValidationContext) []framework.ValidationRule {
