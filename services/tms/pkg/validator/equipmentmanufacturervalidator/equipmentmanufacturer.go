...
type Validator struct {
	factory *framework.TenantedValidatorFactory[*equipmentmanufacturer.EquipmentManufacturer]
}

func NewValidator(p ValidatorParams) *Validator {
	factory := framework.NewTenantedValidatorFactory[*equipmentmanufacturer.EquipmentManufacturer](
		func(ctx context.Context) (*bun.DB, error) {
			return p.DB.DB(ctx)
		},
	).
		WithModelName("EquipmentManufacturer").
		WithUniqueFields(func(entity *equipmentmanufacturer.EquipmentManufacturer) []framework.UniqueField {
			return []framework.UniqueField{
				{
					Name:     "name",
					GetValue: func() string { return entity.Name },
					Message:  "Equipment manufacturer with name ':value' already exists in the organization.",
				},
			}
		}).
		WithCustomRules(func(entity *equipmentmanufacturer.EquipmentManufacturer, vc *validator.ValidationContext) []framework.ValidationRule {
