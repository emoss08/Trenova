...
type Validator struct {
	factory *framework.TenantedValidatorFactory[*accounting.AccountType]
}

func NewValidator(p ValidatorParams) *Validator {
	factory := framework.NewTenantedValidatorFactory[*accounting.AccountType](
		func(ctx context.Context) (*bun.DB, error) {
			return p.DB.DB(ctx)
		},
	).
		WithModelName("AccountType").
		WithUniqueFields(func(entity *accounting.AccountType) []framework.UniqueField {
			return []framework.UniqueField{
				{
					Name:     "code",
					GetValue: func() string { return entity.Code },
					Message:  "Account type with code ':value' already exists in the organization.",
				},
			}
		}).
		WithCustomRules(func(entity *accounting.AccountType, vc *validator.ValidationContext) []framework.ValidationRule {
