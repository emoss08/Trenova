...
type Validator struct {
	factory *framework.TenantedValidatorFactory[*accessorialcharge.AccessorialCharge]
}

func NewValidator(p ValidatorParams) *Validator {
	factory := framework.NewTenantedValidatorFactory[*accessorialcharge.AccessorialCharge](
		func(ctx context.Context) (*bun.DB, error) {
			return p.DB.DB(ctx)
		},
	).
		WithModelName("AccessorialCharge").
		WithUniqueFields(func(entity *accessorialcharge.AccessorialCharge) []framework.UniqueField {
			return []framework.UniqueField{
				{
					Name:     "code",
					GetValue: func() string { return entity.Code },
					Message:  "Accessorial charge with code ':value' already exists in the organization.",
				},
			}
		}).WithCustomRules(func(entity *accessorialcharge.AccessorialCharge, vc *validator.ValidationContext) []framework.ValidationRule {
