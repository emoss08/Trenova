...
type DataFetcher[T any] func(ctx context.Context, authCtx *authCtx.AuthContext, lastTimestamp int64) ([]T, int64, error)
...
// Stream starts the SSE stream on the given Gin context
//
//nolint:gocognit // This is a complex function, but it's not too bad
func (s *SSEStreamer[T]) Stream(c *gin.Context) {
...
func StreamWithSimplePoller[T any](
	c *gin.Context,
	config SSEConfig,
	fetchFunc func(ctx context.Context, authCtx *authCtx.AuthContext) ([]T, error),
	timestampFunc func(item T) int64,
) {
	dataFetcher := func(ctx context.Context, authContext *authCtx.AuthContext, lastTimestamp int64) ([]T, int64, error) {
		items, err := fetchFunc(ctx, authContext)
		if err != nil {
