...
func (te *TracingEvaluator) EvaluateWithTrace( //nolint:funlen // only at 52
	ctx context.Context,
	expr string,
	...
	for _, varName := range vars {
		...
		if variable, vErr := te.evaluator.variables.Get(varName); vErr == nil {
			if value, valErr := variable.Resolve(varCtx); valErr == nil {
				varStep.Result = Success
				varStep.Value = value
			} else {
				varStep.Result = fmt.Sprintf("Failed: %v", valErr)
			}
		} else {
	...
	trace = append(trace, evalStep, TraceStep{
...
func (te *TracingEvaluator) evaluateNode( //nolint:gocognit,cyclop,funlen // this is fine
	node Node,
	ctx *tracingEvaluationContext,
) (any, error) {
	nodeTrace := TraceStep{
