...
func TestTemplateTranslatesText(t *testing.T) {
	...
	assert.Equal(
		t,
		"New load assigned",
		renderWith(t, i18n.EN, ChannelNotificationBody, source, nil),
	)
	assert.Equal(
		t,
		"Nueva carga asignada",
		renderWith(t, i18n.ES, ChannelNotificationBody, source, nil),
	)
...
func TestTemplateTranslatesWithArguments(t *testing.T) {
	...
	assert.Equal(
		t,
		"Se le asignó la carga P-10482.",
		renderWith(t, i18n.ES, ChannelNotificationBody, source, data),
	)
	assert.Equal(
		t,
		"已指派貨載 P-10482 給您。",
		renderWith(t, i18n.ZhTW, ChannelNotificationBody, source, data),
	)
}
...
func TestTemplatePluralFollowsTheLocale(t *testing.T) {
	...
	assert.Equal(
		t,
		"It has 1 stop.",
		renderWith(t, i18n.EN, ChannelNotificationBody, source, map[string]any{"Stops": 1}),
	)
	assert.Equal(
		t,
		"It has 4 stops.",
		renderWith(t, i18n.EN, ChannelNotificationBody, source, map[string]any{"Stops": 4}),
	)
	assert.Equal(
		t,
		"Tiene 1 parada.",
		renderWith(t, i18n.ES, ChannelNotificationBody, source, map[string]any{"Stops": 1}),
	)
	assert.Equal(
		t,
		"Tiene 4 paradas.",
		renderWith(t, i18n.ES, ChannelNotificationBody, source, map[string]any{"Stops": 4}),
	)
	...
	assert.Equal(
		t,
		"共有 1 個停靠點。",
		renderWith(t, i18n.ZhTW, ChannelNotificationBody, source, map[string]any{"Stops": 1}),
	)
	assert.Equal(
		t,
		"共有 4 個停靠點。",
		renderWith(t, i18n.ZhTW, ChannelNotificationBody, source, map[string]any{"Stops": 4}),
	)
}
