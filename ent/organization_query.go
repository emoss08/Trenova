...
// QueryBillingControl chains the current query on the "billing_control" edge.
func (oq *OrganizationQuery) QueryBillingControl() *BillingControlQuery {
	query := (&BillingControlClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryEmailControl chains the current query on the "email_control" edge.
func (oq *OrganizationQuery) QueryEmailControl() *EmailControlQuery {
	query := (&EmailControlClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithBillingControl tells the query-builder to eager-load the nodes that are connected to
// the "billing_control" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrganizationQuery) WithBillingControl(opts ...func(*BillingControlQuery)) *OrganizationQuery {
	query := (&BillingControlClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithEmailControl tells the query-builder to eager-load the nodes that are connected to
// the "email_control" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrganizationQuery) WithEmailControl(opts ...func(*EmailControlQuery)) *OrganizationQuery {
	query := (&EmailControlClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withEmailControl = query
	return oq
}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withBillingControl; query != nil {
		if err := oq.loadBillingControl(ctx, query, nodes, nil,
			func(n *Organization, e *BillingControl) { n.Edges.BillingControl = e }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrganizationQuery) loadEmailControl(ctx context.Context, query *EmailControlQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *EmailControl)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
