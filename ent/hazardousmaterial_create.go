...
func (hmc *HazardousMaterialCreate) SetPackingGroup(hg hazardousmaterial.PackingGroup) *HazardousMaterialCreate {
	hmc.mutation.SetPackingGroup(hg)
	return hmc
}

// SetNillablePackingGroup sets the "packing_group" field if the given value is not nil.
func (hmc *HazardousMaterialCreate) SetNillablePackingGroup(hg *hazardousmaterial.PackingGroup) *HazardousMaterialCreate {
	if hg != nil {
		hmc.SetPackingGroup(*hg)
	}
	return hmc
}
...
func (hmc *HazardousMaterialCreate) AddCommodityIDs(ids ...uuid.UUID) *HazardousMaterialCreate {
	hmc.mutation.AddCommodityIDs(ids...)
	return hmc
}

// AddCommodities adds the "commodities" edges to the Commodity entity.
func (hmc *HazardousMaterialCreate) AddCommodities(c ...*Commodity) *HazardousMaterialCreate {
	ids := make([]uuid.UUID, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return hmc.AddCommodityIDs(ids...)
}
...
func (hmc *HazardousMaterialCreate) check() error {
	...
	if v, ok := hmc.mutation.PackingGroup(); ok {
...
func (hmc *HazardousMaterialCreate) createSpec() (*HazardousMaterial, *sqlgraph.CreateSpec) {
	...
	if nodes := hmc.mutation.CommoditiesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
