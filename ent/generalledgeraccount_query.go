...
func (glaq *GeneralLedgerAccountQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*GeneralLedgerAccount, init func(*GeneralLedgerAccount), assign func(*GeneralLedgerAccount, *Tag)) error {
	...
	query.Where(predicate.Tag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(generalledgeraccount.TagsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.general_ledger_account_tags
		if fk == nil {
			return fmt.Errorf(`foreign-key "general_ledger_account_tags" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "general_ledger_account_tags" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
