...
// QueryCommodities chains the current query on the "commodities" edge.
func (hmq *HazardousMaterialQuery) QueryCommodities() *CommodityQuery {
...
func (hmq *HazardousMaterialQuery) Clone() *HazardousMaterialQuery {
	...
	return &HazardousMaterialQuery{
		...
		withCommodities:  hmq.withCommodities.Clone(),
...
// WithCommodities tells the query-builder to eager-load the nodes that are connected to
// the "commodities" edge. The optional arguments are used to configure the query builder of the edge.
func (hmq *HazardousMaterialQuery) WithCommodities(opts ...func(*CommodityQuery)) *HazardousMaterialQuery {
	query := (&CommodityClient{config: hmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	hmq.withCommodities = query
	return hmq
}
...
func (hmq *HazardousMaterialQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*HazardousMaterial, error) {
	...
	if query := hmq.withCommodities; query != nil {
...
func (hmq *HazardousMaterialQuery) loadCommodities(ctx context.Context, query *CommodityQuery, nodes []*HazardousMaterial, init func(*HazardousMaterial), assign func(*HazardousMaterial, *Commodity)) error {
	...
	query.withFKs = true
	query.Where(predicate.Commodity(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(hazardousmaterial.CommoditiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.hazardous_material_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "hazardous_material_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "hazardous_material_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
