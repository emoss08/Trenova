...
| Inbound polling | Implemented | Temporal schedule `edi-inbound-poll` (every 2 minutes, overlap skip) polls every active SFTP/VAN profile that has an `inboundDirectory` configured and a partner assigned. `POST /edi/communication-profiles/{id}/poll/` triggers a synchronous poll-and-process for one profile on demand (ops "poll now"). |
