...
perform_login() {
    ...
    local login_payload='{"emailAddress": "admin@trenova.app", "password": "admin"}'
    ...
    if echo "$login_response" | jq -e '.sessionId and .user' >/dev/null 2>&1; then
        ...
        if [ -n "$cookie_value" ]; then
            local extracted_cookie=$(echo "$cookie_value" | sed -n "s/.*$SESSION_COOKIE_NAME=\([^;]*\).*/\1/p")
        ...
        rm -f "$headers_file"
        local error_msg=$(echo "$login_response" | jq -r '.message // .error // "Unknown error"' 2>/dev/null || echo "Invalid JSON response")
...
execute_request() {
    ...
    # Execute request
    local curl_output=$(curl -s -w "%{http_code},%{time_total},%{size_download},%{size_upload}" \
    ...
    if [ $curl_exit_code -eq 0 ] && [[ $curl_output =~ ^[0-9]+,[0-9.]+,[0-9]+,[0-9]+$ ]]; then
        IFS=',' read -r http_code time_total size_download size_upload <<< "$curl_output"
...
run_ramp_test() {
    ...
    local sleep_interval=$(echo "scale=3; 1 / $requests_per_second" | bc -l)
...
analyze_results() {
    ...
    local successful_requests=$(awk '$1 == 200 { count++ } END { print count+0 }' "$RESULTS_FILE")
    ...
    local response_times=$(awk '$1 == 200 { print $2 }' "$RESULTS_FILE" | sort -n)
    
    if [ -n "$response_times" ]; then
        local avg_time=$(echo "$response_times" | awk '{ sum += $1; count++ } END { if (count > 0) printf "%.3f", sum/count; else print "0" }')
        local min_time=$(echo "$response_times" | head -n1)
        local max_time=$(echo "$response_times" | tail -n1)
        
        # Calculate percentiles
        local total_successful=$(echo "$response_times" | wc -l)
        local p50_index=$(echo "($total_successful * 0.5) / 1" | bc)
        local p90_index=$(echo "($total_successful * 0.9) / 1" | bc)
        local p95_index=$(echo "($total_successful * 0.95) / 1" | bc)
        local p99_index=$(echo "($total_successful * 0.99) / 1" | bc)
        
        local p50=$(echo "$response_times" | sed -n "${p50_index}p")
        local p90=$(echo "$response_times" | sed -n "${p90_index}p")
        local p95=$(echo "$response_times" | sed -n "${p95_index}p")
        local p99=$(echo "$response_times" | sed -n "${p99_index}p")
    else
        local avg_time="N/A"
        local min_time="N/A"
        local max_time="N/A"
    ...
    # Data transfer
    local total_mb_sent=$(echo "scale=2; $TOTAL_BYTES_SENT / 1024 / 1024" | bc -l)
    local total_mb_received=$(echo "scale=2; $TOTAL_BYTES_RECEIVED / 1024 / 1024" | bc -l)
    
    # Error analysis
    local error_distribution=$(awk '$1 != 200 { errors[$1]++ } END { for (code in errors) printf "%s: %d\\n", code, errors[code] }' "$RESULTS_FILE" | sort)
...
run_stress_test() {
    ...
    local endpoint="${TEST_SCENARIOS[$endpoint_key]}"
    local timestamp=$(date +%Y%m%d_%H%M%S)
    local result_file="${RESULTS_DIR}/${endpoint_key}/result_${pattern}_${timestamp}.txt"
    ...
    local analysis_output=$(analyze_results "$endpoint_key" "$pattern")
