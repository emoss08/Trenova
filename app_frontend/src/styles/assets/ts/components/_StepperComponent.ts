...
class StepperComponent {
  ...
  constructor(_element: HTMLElement, options: IStepperOptions) {
    ...
    // Elements
    this.steps = this.element.querySelectorAll('[data-mt-stepper-element="nav"]')
    this.btnNext = this.element.querySelector('[data-mt-stepper-action="next"]')
    this.btnPrev = this.element.querySelector('[data-mt-stepper-action="previous"]')
    this.btnSubmit = this.element.querySelector('[data-mt-stepper-action="submit"]')
  ...
  private initHandlers = () => {
    ...
    DOMEventHandlerUtil.on(this.element, '[data-mt-stepper-action="step"]', 'click', (e: Event) => {
      e.preventDefault()
  ...
  private getStepContent = (index: number) => {
    const content = this.element.querySelectorAll('[data-mt-stepper-element="content"]')
    if (!content) {
      return false
    }
  ...
  private refreshUI = () => {
    ...
    const elements = this.element.querySelectorAll(
      '[data-mt-stepper-element="nav"], [data-mt-stepper-element="content"], [data-mt-stepper-element="info"]'
    )
    ...
    for (var i = 0, len = elements.length; i < len; i++) {
      ...
      if (index === this.currentStepIndex) {
        ...
        if (
          ...
          element.getAttribute('data-mt-stepper-element') === 'content'
        ) {
  ...
  public static bootstrap(attr: string = '[data-mt-stepper]') {
