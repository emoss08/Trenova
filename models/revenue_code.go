...
func (rc *RevenueCode) SetOrgID(orgID uuid.UUID) {
	rc.OrganizationID = orgID
}

func (rc *RevenueCode) SetBuID(buID uuid.UUID) {
	rc.BusinessUnitID = buID
}

func (rc *RevenueCode) validateRevenueCode() error {
	if rc.ExpenseAccountID != nil && rc.ExpenseAccount.AccountType != RevenueAccountType {
		return errExpenseAccountMustBeExpense
	}

	if rc.RevenueAccountID != nil && rc.RevenueAccount.AccountType != RevenueAccountType {
...
func (rc *RevenueCode) FetchRevenueCodesForOrg(db *gorm.DB, orgID, buID uuid.UUID, offset, limit int) ([]RevenueCode, int64, error) {
	...
	if err := db.
		Model(&RevenueCode{}).Where("organization_id = ? AND business_unit_id = ?", orgID, buID).Count(&totalRows).Error; err != nil {
	...
	if err := db.
		...
		Find(&revenueCodes).Error; err != nil {
		return revenueCodes, 0, err
	}
...
func (rc *RevenueCode) FetchRevenueCodeDetails(db *gorm.DB, orgID, buID uuid.UUID, id string) (RevenueCode, error) {
	...
	if err := db.
		Model(&RevenueCode{}).Where("organization_id = ? AND id = ? AND business_unit_id = ?", orgID, id, buID).
		First(&revenueCode).Error; err != nil {
