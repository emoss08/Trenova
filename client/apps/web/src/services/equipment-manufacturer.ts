...
export class EquipmentManufacturerService {
  public async bulkUpdateStatus(request: BulkUpdateEquipmentManufacturerStatusRequest) {
    const response = await api.post<BulkUpdateEquipmentManufacturerStatusResponse>(
    ...
    return safeParse(
      bulkUpdateEquipmentManufacturerStatusResponseSchema,
      response,
      "BulkUpdateEquipmentManufacturerStatus",
    );
  }

  public async patch(id: EquipmentManufacturer["id"], data: Partial<EquipmentManufacturer>) {
    const response = await api.patch<EquipmentManufacturer>(`/equipment-manufacturers/${id}`, data);
