...
import { InvoiceReconciliationPage } from "../page";
...
vi.mock("@/services/api", () => ({
  apiService: {
    invoiceAdjustmentService: {
      listReconciliationExceptions: mocks.listReconciliationExceptions,
...
beforeEach(() => {
  mocks.listReconciliationExceptions.mockResolvedValue({ results: [], count: 0 });
...
describe("reconciliation exceptions empty states", () => {
  it("says what raises an exception when none is open, and draws the review pane", async () => {
    renderPage();
  ...
  it("offers to clear the filters when they are what emptied the list", async () => {
    ...
    renderPage("?status=Resolved&query=ACME", onUrlUpdate);
