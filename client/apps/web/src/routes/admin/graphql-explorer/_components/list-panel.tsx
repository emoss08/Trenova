...
function KindGlyph({ kind }: { kind: GlyphKind }) {
  const config: Record<GlyphKind, { letter: string; className: string }> = {
    ...
    mutation: { letter: "M", className: "bg-warning-subtle text-warning-foreground" },
    subscription: {
      letter: "S",
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active && "ring-primary/30 ring-1 ring-inset",
      )}
    >
      <KindGlyph kind={kind} />
      ...
      <span className="text-2xs text-muted-foreground/70 shrink-0">{domain}</span>
    </button>
  );
}
...
export function ListPanel({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="text-2xs text-muted-foreground/70 flex items-center gap-3 border-t px-3 py-1.5">
