...
export function createIftaMileageEntryFormSchema(today: number) {
  return z.object({
    tractorId: z.string().min(1, { message: "Choose the tractor that ran the miles" }),
    jurisdictionId: z.string().min(1, { message: "Choose the jurisdiction the miles were run in" }),
    traveledAt: z
      .number()
      .int()
      .positive({ message: "Enter the day the miles were run" })
      .max(today, { message: "Miles cannot be run in the future" }),
