...
type ViewDelayCodeModalFormProps = {
  delayCode: DelayCode;
};

function ViewDelayCodeModalForm({ delayCode }: ViewDelayCodeModalFormProps) {
    ...
    <Box className={classes.div}>
      <Box>
        ...
        <Switch
          ...
          defaultChecked={delayCode.fCarrierOrDriver}
          readOnly
        />
        <Group position="right" mt="md">
          <Button
            ...
            onClick={() => {
              store.set("selectedRecord", delayCode);
...
export function ViewDelayCodeModal() {
  ...
  return (
    <Modal.Root opened={showViewModal} onClose={() => setShowViewModal(false)}>
      ...
      <Modal.Content>
        <Modal.Header>
          <Modal.Title>View Delay code</Modal.Title>
          <Modal.CloseButton />
        </Modal.Header>
        <Modal.Body>
          <Suspense fallback={<Skeleton height={400} />}>
            {delayCode && <ViewDelayCodeModalForm delayCode={delayCode} />}
          </Suspense>
        </Modal.Body>
      </Modal.Content>
    </Modal.Root>
