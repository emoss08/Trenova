...
interface EditJobTitleFormValues {
  name: string;
  description: string;
  status: string;
  job_function: string;
...
export const EditJobTitleModalForm: React.FC<Props> = ({ jobTitle }) => {
  ...
  const form = useForm<EditJobTitleFormValues>({
    validate: yupResolver(editJobTitleSchema),
    initialValues: {
  ...
  const submitForm = (values: EditJobTitleFormValues) => {
    setLoading(true);
