...
type CustomerBillingProfileFormProps = {
  billingProfile: CustomerBillingProfile;
};

function CustomerBillingProfileForm({
  billingProfile,
}: CustomerBillingProfileFormProps) {
  ...
  const mutation = useMutation(
    (values: CustomerBillingProfileFormValues) =>
      axios.patch(`/customer_billing_profiles/${billingProfile.id}/`, values),
    {
      onSuccess: () => {
        queryClient
          .invalidateQueries({
