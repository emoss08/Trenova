...
export const ExportGLAccountModal: React.FC = ({}) => {
  const [loading, setLoading] = React.useState<boolean>(false);
  const [showExportModal, setShowExportModal] =
    generalLedgerTableStore.use("exportModalOpen");

  const { data: columnsData, isLoading: isColumnsLoading } = useQuery({
    queryKey: ["gl-account-columns"],
    queryFn: () => getColumns("GeneralLedgerAccount"),
  ...
  return (
    <>
      ...
      >
        ...
        <Modal.Content>
          <Modal.Header>
            <Modal.Title>Export GL Accounts</Modal.Title>
            <Modal.CloseButton />
          </Modal.Header>
          <Modal.Body>
            {isColumnsLoading ? (
                ...
                <form onSubmit={form.onSubmit((values) => submitForm(values))}>
                  <Box mb={10}>
                    ...
                    <Text size="xs" color="dimmed" mt={5}>
                      ...
                      'organization__name' is the name field of the organization
