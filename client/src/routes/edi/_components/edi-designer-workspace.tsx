      ...
      <main className="flex min-h-0 flex-col rounded-md border bg-background">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[280px_minmax(0,1fr)]">
          ...
          <Tabs defaultValue="elements" className="min-h-0 gap-0">
            <div className="flex items-center justify-between border-b bg-sidebar px-1">
...
function DocumentPreviewArchiveTab() {
  ...
  const [profileId, setProfileId] = useState("");
  const [shipmentId, setShipmentId] = useState("");
  const [archivePartnerId, setArchivePartnerId] = useState("");
  const [archiveTransactionSet, setArchiveTransactionSet] = useState("204");
  const [archiveDirection, setArchiveDirection] = useState("Outbound");
  const [archiveStatus, setArchiveStatus] = useState("");
  const [archiveGeneratedFrom, setArchiveGeneratedFrom] = useState("");
  const [archiveGeneratedTo, setArchiveGeneratedTo] = useState("");
  const [archiveQuery, setArchiveQuery] = useState("");
  const [inspectorMessageId, setInspectorMessageId] = useState("");
  ...
  const messagesQueryString = useMemo(() => {
    ...
    if (archiveTransactionSet) params.set("transactionSet", archiveTransactionSet);
    if (archiveDirection) params.set("direction", archiveDirection);
    if (archivePartnerId) params.set("partnerId", archivePartnerId);
    if (archiveStatus) params.set("status", archiveStatus);
    if (archiveQuery.trim()) params.set("query", archiveQuery.trim());
    const generatedFrom = dateInputToUnix(archiveGeneratedFrom, false);
    const generatedTo = dateInputToUnix(archiveGeneratedTo, true);
          ...
          <TabsContent value="archive" className="min-h-0">
            <MessageArchive
              ...
              onFiltersChange={(patch) => {
                if (patch.partnerId !== undefined) setArchivePartnerId(patch.partnerId);
                if (patch.transactionSet !== undefined) {
                  setArchiveTransactionSet(patch.transactionSet);
                }
                if (patch.direction !== undefined) setArchiveDirection(patch.direction);
                if (patch.status !== undefined) setArchiveStatus(patch.status);
                if (patch.generatedFrom !== undefined) setArchiveGeneratedFrom(patch.generatedFrom);
                if (patch.generatedTo !== undefined) setArchiveGeneratedTo(patch.generatedTo);
                if (patch.query !== undefined) setArchiveQuery(patch.query);
              }}
              onOpenMessage={setInspectorMessageId}
            />
          </TabsContent>
        </Tabs>
      </main>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        disabled={!message.rawX12}
                        onClick={() =>
                          downloadText(buildX12Filename(message), message.rawX12, "text/plain")
                        }
                      >
            ...
            >
              <div className="mb-2 flex items-center gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    downloadJson(buildMessageJsonFilename(message), message.payloadSnapshot)
                  }
                >
      ...
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <Button
            type="button"
            variant="outline"
            onClick={() => downloadText(buildX12Filename(message), message.rawX12, "text/plain")}
          >
            <DownloadIcon className="size-4" />
            Download
          </Button>
...
function dateInputToUnix(value: string, endOfDay: boolean) {
