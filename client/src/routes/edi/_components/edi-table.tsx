...
function DesignerWorkspace() {
  ...
  const [profileDraft, setProfileDraft] = useState<UpsertEDIPartnerDocumentProfileRequest>({
    ediPartnerId: "",
  ...
  const profilesQuery = useQuery(queries.edi.documentProfiles("?limit=100"));
  const templatesQuery = useQuery(queries.edi.templates("?limit=50"));
  const messagesQuery = useQuery(queries.edi.messages("?limit=25"));

  const selectedProfile = useMemo(
    () => profilesQuery.data?.results.find((profile) => profile.id === profileId),
    [profileId, profilesQuery.data?.results],
  );
  const activeTemplate = useMemo(
    () =>
      templatesQuery.data?.results.find((template) => template.id === profileDraft.templateId) ??
      templatesQuery.data?.results[0],
    [profileDraft.templateId, templatesQuery.data?.results],
  );
  const activeVersion = selectedProfile?.templateVersion ?? activeTemplate?.versions[0];
  const segments = useMemo(() => activeVersion?.segments ?? [], [activeVersion?.segments]);
  ...
  useEffect(() => {
    if (segments.length > 0 && !segments.some((segment) => segment.id === selectedSegmentId)) {
  ...
  return (
    <div className="grid min-h-[calc(100vh-11rem)] grid-cols-[320px_minmax(0,1fr)_360px] gap-3">
      <aside className="flex min-h-0 flex-col rounded-md border bg-background">
        <DesignerPanelHeader icon={<FileCode2Icon />} title="204 Profile" />
        <div className="flex min-h-0 flex-col gap-3 overflow-auto p-3">
          <SelectBlock
            ...
            options={(partnersQuery.data?.results ?? [])
              ...
              .map((partner) => ({ value: partner.id, label: `${partner.code} - ${partner.name}` }))}
          />
          <SelectBlock
          ...
          <div className="space-y-1">
            <LabelText>Partner Settings</LabelText>
            <Textarea
              value={rawPartnerSettings}
              onChange={(event) => setRawPartnerSettings(event.target.value)}
      ...
      <main className="flex min-h-0 flex-col rounded-md border bg-background">
        <div className="flex items-center justify-between gap-3 border-b px-3 py-2">
          <div>
            <div className="text-sm font-semibold">Structured 204 Template</div>
            <div className="text-xs text-muted-foreground">
              {activeVersion?.x12Version ?? "004010"} / {segments.length} segment definitions
            </div>
          </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="grid min-h-0 grid-rows-[minmax(0,1fr)_260px]">
            <ElementInspector segment={selectedSegment} />
      ...
      <aside className="flex min-h-0 flex-col rounded-md border bg-background">
        <DesignerPanelHeader icon={<DatabaseIcon />} title="204 Archive" />
...
function DesignerPanelHeader({ icon, title }: { icon: ReactNode; title: string }) {
  return (
  ...
  return (
    <div className="space-y-1">
      ...
      <select
        ...
        className="h-7 w-full rounded-md border border-input bg-muted px-2 text-sm outline-none focus-visible:border-brand focus-visible:ring-4 focus-visible:ring-brand/30"
      >
  ...
  return (
    <div className="space-y-2 rounded-md border bg-muted/30 p-2">
      ...
      <div className="grid grid-cols-4 gap-2">
        <InputBlock
          label="Element"
          value={envelope.elementSeparator}
          onChange={(value) => update("elementSeparator", value)}
        />
        <InputBlock
          label="Segment"
          value={envelope.segmentTerminator}
          onChange={(value) => update("segmentTerminator", value)}
        />
        <InputBlock
          label="Component"
          value={envelope.componentSeparator}
          onChange={(value) => update("componentSeparator", value)}
        />
        <InputBlock
          label="Usage"
          value={envelope.interchangeUsageIndicator}
          onChange={(value) => update("interchangeUsageIndicator", value)}
        />
      </div>
    </div>
  );
    ...
    <div className="min-h-0 overflow-auto border-r">
      {segments.map((segment) => (
        <button
          ...
          className={cn(
            "flex w-full items-center justify-between border-b px-3 py-2 text-left text-sm hover:bg-muted",
            selectedSegmentId === segment.id && "bg-muted",
          )}
        >
          <span className="font-mono font-medium">{segment.segmentId}</span>
          <span className="truncate pl-3 text-xs text-muted-foreground">{segment.name}</span>
        </button>
      ))}
    </div>
  );
...
function ElementInspector({ segment }: { segment?: EDITemplateSegment }) {
  if (!segment) {
    return <div className="p-4 text-sm text-muted-foreground">No segment selected.</div>;
  }
  return (
      ...
      <Table>
        ...
        <TableBody>
          {segment.elements.map((element) => (
            <TableRow key={`${segment.id}-${element.position}`}>
              <TableCell className="font-mono">{element.position}</TableCell>
              <TableCell>{element.name}</TableCell>
              <TableCell>
                <Badge variant={element.validation.required ? "warning" : "outline"}>
                  {element.source}
                </Badge>
              </TableCell>
...
function PreviewPane({
  ...
  return (
    <div className="grid min-h-0 grid-cols-[minmax(0,1fr)_280px] border-t">
...
function DiagnosticsList({ diagnostics }: { diagnostics: EDIDiagnostic[] }) {
  return (
    <div className="min-h-0 overflow-auto border-l p-3">
      <div className="mb-2 text-xs font-semibold">Diagnostics</div>
      {diagnostics.length === 0 ? (
        <div className="text-xs text-muted-foreground">No diagnostics.</div>
      ) : (
        <div className="space-y-2">
          {diagnostics.map((diagnostic, index) => (
            <div key={`${diagnostic.code}-${index}`} className="rounded-md border p-2">
              <div className="flex items-center gap-2">
                <Badge
                  variant={diagnostic.severity === "Error" ? "inactive" : "warning"}
                >
                  {diagnostic.severity}
                </Badge>
...
function profileToDraft(profile: EDIPartnerDocumentProfile): UpsertEDIPartnerDocumentProfileRequest {
  return {
