...
func DotEnvTryLoad(absolutePathToEnvFile string, setEnvFn envSetter) {
	...
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			log.Error().Err(err).Str("envFile", absolutePathToEnvFile).Msg("error parsing .env file")
		}
	} else {
		zap.L().Warn("ENV variables overridden by .env file", zap.String("envFile", absolutePathToEnvFile))
	}
}
