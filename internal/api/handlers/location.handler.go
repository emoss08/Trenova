...
func (h LocationHandler) Get() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			h.logger.Error().Err(err).Msg("Error getting locations")
...
func (h LocationHandler) GetByID() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		entity, err := h.service.Get(c.UserContext(), uuid.MustParse(locationID), ids.OrganizationID, ids.BusinessUnitID)
		if err != nil {
			h.logger.Error().Str("locationID", locationID).Err(err).Msg("Error getting location by ID")
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			})
...
func (h LocationHandler) Create() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err = h.permissionService.CheckUserPermission(c, constants.EntityLocation, "create"); err != nil {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Error{
				Code:    fiber.StatusForbidden,
				Message: err.Error(),
			})
		...
		attemptID := h.auditService.LogAttempt(c.Context(), constants.TableLocation, "", property.AuditLogActionCreate, createdEntity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)

		entity, err := h.service.Create(c.UserContext(), createdEntity)
		if err != nil {
			h.logger.Error().Interface("entity", createdEntity).Err(err).Msg("Failed to create Location")
		...
		h.auditService.LogAction(c.Context(), constants.TableLocation, entity.ID.String(), property.AuditLogActionCreate, entity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
...
func (h LocationHandler) Update() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err = h.permissionService.CheckUserPermission(c, constants.EntityLocation, "update"); err != nil {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Error{
				Code:    fiber.StatusForbidden,
				Message: err.Error(),
			})
		...
		attemptID := h.auditService.LogAttempt(c.Context(), constants.TableLocation, locationID, property.AuditLogActionUpdate, updatedEntity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)

		entity, err := h.service.UpdateOne(c.UserContext(), updatedEntity)
		if err != nil {
			h.logger.Error().Interface("entity", updatedEntity).Err(err).Msg("Failed to update Location")
			resp := utils.CreateServiceError(c, err)
		...
		h.auditService.LogAction(c.Context(), constants.TableLocation, entity.ID.String(), property.AuditLogActionUpdate, entity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
