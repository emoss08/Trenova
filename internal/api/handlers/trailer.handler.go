...
func (h TrailerHandler) Get() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err = h.permissionService.CheckUserPermission(c, constants.EntityTrailer, constants.ActionView); err != nil {
		...
		filter := &services.TrailerQueryFilter{
		...
		if err != nil {
			h.logger.Error().Err(err).Msg("Error getting trailers")
		...
		return c.Status(fiber.StatusOK).JSON(types.HTTPResponse[[]*models.Trailer]{
			Results: entities,
			Count:   cnt,
			Next:    nextURL,
			Prev:    prevURL,
	...
	return func(c *fiber.Ctx) error {
		...
		if trailerID == "" {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Error{
				...
				Message: "Trailer ID is required",
			})
		}
		...
		entity, err := h.service.Get(c.UserContext(), uuid.MustParse(trailerID), ids.OrganizationID, ids.BusinessUnitID)
		if err != nil {
			h.logger.Error().Str("trailerID", trailerID).Err(err).Msg("Error getting trailer by ID")
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			})
...
func (h TrailerHandler) Create() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err = h.permissionService.CheckUserPermission(c, constants.EntityTrailer, constants.ActionCreate); err != nil {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Error{
				Code:    fiber.StatusForbidden,
				Message: err.Error(),
			})
		...
		go h.auditService.LogAction(constants.TableTrailer, entity.ID.String(), property.AuditLogActionCreate, entity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
...
func (h TrailerHandler) Update() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		go h.auditService.LogAction(constants.TableTrailer, entity.ID.String(), property.AuditLogActionUpdate, entity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
