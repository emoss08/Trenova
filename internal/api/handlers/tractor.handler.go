...
func (h *TractorHandler) GetTractors() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		err := h.PermissionService.CheckUserPermission(c, "read_tractor")
		if err != nil {
...
func (h *TractorHandler) CreateTractor() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Check if the user has the required permission
		err := h.PermissionService.CheckUserPermission(c, "create_tractor")
		if err != nil {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
				"error":   "Unauthorized",
				"message": "You do not have the required permission to access this resource",
...
func (h *TractorHandler) UpdateTractor() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Check if the user has the required permission
		err := h.PermissionService.CheckUserPermission(c, "update_tractor")
		if err != nil {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
				"error":   "Unauthorized",
				"message": "You do not have the required permission to access this resource",
