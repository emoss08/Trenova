...
func (h *Handler) AcceptSuggestion(c *fiber.Ctx) error {
	...
	var req dedicatedlane.SuggestionAcceptRequest
...
func (h *Handler) RejectSuggestion(c *fiber.Ctx) error {
	...
	var req dedicatedlane.SuggestionRejectRequest
	if err := c.BodyParser(&req); err != nil {
		return h.eh.HandleError(c, err)
	}
...
func (h *Handler) AnalyzePatterns(c *fiber.Ctx) error {
	...
	var req dedicatedlane.PatternAnalysisRequest
	if err := c.BodyParser(&req); err != nil {
		return h.eh.HandleError(c, err)
	}
	...
	result, err := h.ss.AnalyzePatterns(c.UserContext(), &req, reqCtx.UserID)
	if err != nil {
		return h.eh.HandleError(c, err)
	}
