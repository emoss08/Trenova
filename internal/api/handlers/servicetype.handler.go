...
func (h ServiceTypeHandler) Get() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err = h.permissionService.CheckUserPermission(c, constants.EntityServiceType, constants.ActionView); err != nil {
		...
		return c.Status(fiber.StatusOK).JSON(types.HTTPResponse[[]*models.ServiceType]{
			Results: entities,
			Count:   cnt,
			Next:    nextURL,
			Prev:    prevURL,
...
func (h ServiceTypeHandler) Create() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err = h.permissionService.CheckUserPermission(c, constants.EntityServiceType, constants.ActionCreate); err != nil {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Error{
				Code:    fiber.StatusForbidden,
				Message: err.Error(),
			})
		...
		attemptID := h.auditService.LogAttempt(c.Context(), constants.TableServiceType, "", property.AuditLogActionCreate, createdEntity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
		...
		h.auditService.LogAction(c.Context(), constants.TableServiceType, entity.ID.String(), property.AuditLogActionCreate, entity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
...
func (h ServiceTypeHandler) Update() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err = h.permissionService.CheckUserPermission(c, constants.EntityServiceType, constants.ActionUpdate); err != nil {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Error{
				Code:    fiber.StatusForbidden,
				Message: err.Error(),
			})
		...
		attemptID := h.auditService.LogAttempt(c.Context(), constants.TableServiceType, serviceTypeID, property.AuditLogActionUpdate, updatedEntity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
		...
		h.auditService.LogAction(c.Context(), constants.TableServiceType, entity.ID.String(), property.AuditLogActionUpdate, entity, ids.UserID, ids.OrganizationID, ids.BusinessUnitID)
