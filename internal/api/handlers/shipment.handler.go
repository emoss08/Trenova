...
func (h *ShipmentHandler) RegisterRoutes(r fiber.Router) {
	...
	api.Get("/", h.Get())
	api.Get("/count", h.GetShipmentCountByStatus())
	api.Get("/:shipmentID", h.GetByID())
	api.Post("/", h.Create())
	// api.Put("/:shipmentID", h.Update())
}

func (h *ShipmentHandler) GetShipmentCountByStatus() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if isHazardous := c.Query("isHazardous"); isHazardous != "" {
			...
			} else if isHazardous == "false" {
				filter.IsHazardous = false
			}
		}

		counts, totalCount, err := h.service.GetShipmentCountByStatus(c.UserContext(), filter)
		if err != nil {
		...
		return c.Status(fiber.StatusOK).JSON(types.HTTPResponse[[]services.ShipmentCountByStatus]{
			Count:   totalCount,
			Results: counts,
		})
	}
