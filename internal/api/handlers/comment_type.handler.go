...
func (h *CommentTypeService) GetCommentTypes() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		err := h.PermissionService.CheckUserPermission(c, "read_commenttype")
		if err != nil {
...
func (h *CommentTypeService) CreateCommentType() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Check if the user has the required permission
		err := h.PermissionService.CheckUserPermission(c, "create_commenttype")
		if err != nil {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
				"error":   "Unauthorized",
				"message": "You do not have the required permission to access this resource",
...
func (h *CommentTypeService) UpdateCommentType() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Check if the user has the required permission
		err := h.PermissionService.CheckUserPermission(c, "update_commenttype")
		if err != nil {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
				"error":   "Unauthorized",
				"message": "You do not have the required permission to access this resource",
