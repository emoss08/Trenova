...
func (h RevenueCodeHandler) Get() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if !ok || !orgOK {
			h.logger.Error().Msg("RevenueCodeHandler: Organization & Business Unit ID not found in context")
		...
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Error{
				...
				Message: "Failed to get RevenueCodes",
			})
		}
...
func (h RevenueCodeHandler) GetByID() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		entity, err := h.service.Get(c.UserContext(), uuid.MustParse(revenuecodeID), orgID, buID)
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: "Failed to get RevenueCode",
			})
		}
