...
func TestNewChargeTypeService(t *testing.T) {
	...
	t.Run("CreateAndGet", func(t *testing.T) {
		created, err := service.Create(ctx, createTestChargeType("OKAYOKAY"), user.ID)
		require.NoError(t, err)
	...
	t.Run("GetAll", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			_, err := service.Create(ctx, createTestChargeType(fmt.Sprintf("CODE%d", i)), user.ID)
			require.NoError(t, err)
		}
