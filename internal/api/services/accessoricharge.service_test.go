...
func TestAccessorialChargeService(t *testing.T) {
	...
	// Helper function to create a test AccessorialCharge
	...
	t.Run("CreateAndGet", func(t *testing.T) {
		created, err := service.Create(ctx, createTestAccessorialCharge("OKAYOKAY"))
		require.NoError(t, err)
	...
	t.Run("GetAll", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			_, err := service.Create(ctx, createTestAccessorialCharge(fmt.Sprintf("CODE%d", i)))
			require.NoError(t, err)
		}
	...
	t.Run("QueryFiltering", func(t *testing.T) {
		...
		assert.Equal(t, "ABC", charges[0].Code)
	})
}
