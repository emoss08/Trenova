...
func TestNewDelayCodeService(t *testing.T) {
	...
	createTestDelayCode := func(code string) *models.DelayCode {
		return &models.DelayCode{
	...
	t.Run("CreateAndGet", func(t *testing.T) {
		created, err := service.Create(ctx, createTestDelayCode("OKAYOKAY"), user.ID)
		require.NoError(t, err)
		assert.NotNil(t, created)
		assert.NotEqual(t, uuid.Nil, created.ID)

		// Get the created DelayCode
	...
	t.Run("GetAll", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			_, err = service.Create(ctx, createTestDelayCode(fmt.Sprintf("CODE%d", i)), user.ID)
			require.NoError(t, err)
		}
	...
	t.Run("Update", func(t *testing.T) {
		// Create a new DelayCode
		newDelayCode := createTestDelayCode("WEIRD")
		created, err := service.Create(ctx, newDelayCode, user.ID)
		require.NoError(t, err)

		// Update the DelayCode
	...
	t.Run("QueryFiltering", func(t *testing.T) {
		// Create DelayCode with different codes
		codes := []string{"ABC", "DEF", "GHI"}
		for _, code := range codes {
			customer := createTestDelayCode(code)
			customer.Code = code
