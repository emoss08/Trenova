...
func TestNewReasonCodeService(t *testing.T) {
	...
	createReasonCode := func(code string) *models.ReasonCode {
		return &models.ReasonCode{
	...
	t.Run("CreateAndGet", func(t *testing.T) {
		created, err := service.Create(ctx, createReasonCode("TEST"), user.ID)
		require.NoError(t, err)
		assert.NotNil(t, created)
		assert.NotEqual(t, uuid.Nil, created.ID)

		// Get the created ReasonCode
	...
	t.Run("GetAll", func(t *testing.T) {
		for i := 0; i < 5; i++ {
			_, err = service.Create(ctx, createReasonCode(fmt.Sprintf("cod%d", i)), user.ID)
			require.NoError(t, err)
		}
	...
	t.Run("Update", func(t *testing.T) {
		// Create a new ReasonCode
		newReasonCode := createReasonCode("TES1")
		created, err := service.Create(ctx, newReasonCode, user.ID)
		require.NoError(t, err)
	...
	t.Run("QueryFiltering", func(t *testing.T) {
		// Create ReasonCode with different codes
		codes := []string{"TES2", "TES3", "TES4"}
		for _, code := range codes {
			entity := createReasonCode(code)
			entity.Code = code
