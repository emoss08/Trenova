...
func (bp *BatchProcessor) shouldSendBatch(userIDStr string, batch []*PendingNotification) (bool, error) {
	if len(batch) == 0 {
		return false, nil
	}
	...
	prefs, err := bp.notificationPrefRepo.GetUserPreferences(bp.ctx, &repositories.GetUserPreferencesRequest{
		UserID:         userID,
...
func (bp *BatchProcessor) sendBatch(userIDStr string, batch []*PendingNotification) {
	...
	for eventType, count := range typeGroups {
		summaryParts = append(summaryParts, fmt.Sprintf("%d %s", count, bp.getEventTypeDisplay(eventType)))
	}
