	...
	err := validation.ValidateStructWithContext(ctx, np,
		...
		validation.Field(&np.UpdateTypes,
			validation.When(
				...
				validation.Required.Error("At least one update type is required when not notifying on all updates"),
			),
		),
		...
		validation.Field(&np.QuietHoursStart,
			validation.When(
				...
				validation.Required.Error("Quiet hours start time is required when quiet hours are enabled"),
			),
		),
		validation.Field(&np.QuietHoursEnd,
			validation.When(
				np.QuietHoursEnabled,
				validation.Required.Error("Quiet hours end time is required when quiet hours are enabled"),
			),
		),
