...
// BillingControlRelationChain provides a fluent API for building nested relationship chains
type BillingControlRelationChain struct {
	relations []string
	options   map[string]func(*bun.SelectQuery) *bun.SelectQuery
}

// NewBillingControlRelationChain creates a new relation chain builder
func NewBillingControlRelationChain() *BillingControlRelationChain {
	return &BillingControlRelationChain{
		relations: []string{},
...
func (rc *BillingControlRelationChain) Add(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *BillingControlRelationChain {
...
func (rc *BillingControlRelationChain) Build() []string {
	return rc.relations
}

// Apply applies the relation chain to a query
func (rc *BillingControlRelationChain) Apply(q *bun.SelectQuery) *bun.SelectQuery {
...
func (b *BillingControlQueryBuilder) WithBusinessUnit() *BillingControlRelationChainBuilder {
	chain := &BillingControlRelationChainBuilder{
		parent: b,
		chain:  NewBillingControlRelationChain(),
	}
...
func (b *BillingControlQueryBuilder) WithOrganization() *BillingControlRelationChainBuilder {
	chain := &BillingControlRelationChainBuilder{
		parent: b,
		chain:  NewBillingControlRelationChain(),
	}
...
// BillingControlRelationChainBuilder provides fluent API for building nested relations
type BillingControlRelationChainBuilder struct {
	parent *BillingControlQueryBuilder
	chain  *BillingControlRelationChain
}

// Load applies the relation chain and returns to the parent builder
func (rb *BillingControlRelationChainBuilder) Load() *BillingControlQueryBuilder {
	rb.parent.query = rb.chain.Apply(rb.parent.query)
	return rb.parent
}

// ThenLoad adds another relation to the chain
func (rb *BillingControlRelationChainBuilder) ThenLoad(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *BillingControlRelationChainBuilder {
	rb.chain.Add(relation, opts...)
	return rb
}

// OrderBy adds ordering to the current relation in the chain
func (rb *BillingControlRelationChainBuilder) OrderBy(order string) *BillingControlRelationChainBuilder {
...
func (rb *BillingControlRelationChainBuilder) Where(condition string, args ...interface{}) *BillingControlRelationChainBuilder {
	if len(rb.chain.relations) > 0 {
		lastRel := rb.chain.relations[len(rb.chain.relations)-1]
		currentOpt := rb.chain.options[lastRel]
		rb.chain.options[lastRel] = func(q *bun.SelectQuery) *bun.SelectQuery {
