...
// QueueRelationChain provides a fluent API for building nested relationship chains
type QueueRelationChain struct {
	relations []string
	options   map[string]func(*bun.SelectQuery) *bun.SelectQuery
}

// NewQueueRelationChain creates a new relation chain builder
func NewQueueRelationChain() *QueueRelationChain {
	return &QueueRelationChain{
		relations: []string{},
...
func (rc *QueueRelationChain) Add(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *QueueRelationChain {
...
func (rc *QueueRelationChain) Build() []string {
	return rc.relations
}

// Apply applies the relation chain to a query
func (rc *QueueRelationChain) Apply(q *bun.SelectQuery) *bun.SelectQuery {
...
func (b *QueueQueryBuilder) WithBusinessUnit() *QueueRelationChainBuilder {
	chain := &QueueRelationChainBuilder{
		parent: b,
		chain:  NewQueueRelationChain(),
	}
...
func (b *QueueQueryBuilder) WithOrganization() *QueueRelationChainBuilder {
	chain := &QueueRelationChainBuilder{
		parent: b,
		chain:  NewQueueRelationChain(),
	}
...
func (b *QueueQueryBuilder) WithProfile() *QueueRelationChainBuilder {
	chain := &QueueRelationChainBuilder{
		parent: b,
		chain:  NewQueueRelationChain(),
	}
...
// WithTemplate creates a relation chain starting with Template
func (b *QueueQueryBuilder) WithTemplate() *QueueRelationChainBuilder {
	chain := &QueueRelationChainBuilder{
		parent: b,
		chain:  NewQueueRelationChain(),
	}
	chain.chain.Add("Template")
	return chain
}

// QueueRelationChainBuilder provides fluent API for building nested relations
type QueueRelationChainBuilder struct {
	parent *QueueQueryBuilder
	chain  *QueueRelationChain
}

// Load applies the relation chain and returns to the parent builder
func (rb *QueueRelationChainBuilder) Load() *QueueQueryBuilder {
	rb.parent.query = rb.chain.Apply(rb.parent.query)
	return rb.parent
}

// ThenLoad adds another relation to the chain
func (rb *QueueRelationChainBuilder) ThenLoad(relation string, opts ...func(*bun.SelectQuery) *bun.SelectQuery) *QueueRelationChainBuilder {
	rb.chain.Add(relation, opts...)
	return rb
}

// OrderBy adds ordering to the current relation in the chain
func (rb *QueueRelationChainBuilder) OrderBy(order string) *QueueRelationChainBuilder {
...
func (rb *QueueRelationChainBuilder) Where(condition string, args ...interface{}) *QueueRelationChainBuilder {
	if len(rb.chain.relations) > 0 {
		lastRel := rb.chain.relations[len(rb.chain.relations)-1]
		currentOpt := rb.chain.options[lastRel]
		rb.chain.options[lastRel] = func(q *bun.SelectQuery) *bun.SelectQuery {
