...
func (hsr *HazmatSegregationRule) Validate(ctx context.Context, multiErr *errors.MultiError) {
	err := validation.ValidateStructWithContext(ctx, hsr,
		...
		validation.Field(&hsr.MinimumDistance,
			validation.When(hsr.SegregationType == SegregationTypeDistance,
				validation.Required.Error("Minimum distance is required when segregation type is Distance"),
		...
		validation.Field(&hsr.DistanceUnit,
			validation.When(hsr.MinimumDistance != nil,
				validation.Required.Error("Distance unit is required when minimum distance is specified"),
				validation.In("ft", "m", "in", "cm").Error("Distance unit must be valid (ft, m, in, cm)"),
			),
		),

		// Exception notes validation when hasExceptions is true
		validation.Field(&hsr.ExceptionNotes,
			validation.When(hsr.HasExceptions,
				validation.Required.Error("Exception notes are required when exceptions are indicated"),
				validation.Length(5, 1000).Error("Exception notes must be between 5 and 1000 characters"),
			),
		),
	)
	...
	if hsr.ClassA != "" && hsr.ClassB != "" && hsr.ClassA == hsr.ClassB {
		if hsr.HazmatAID == nil || hsr.HazmatBID == nil || (hsr.HazmatAID != nil && hsr.HazmatBID != nil && *hsr.HazmatAID == *hsr.HazmatBID) {
			multiErr.Add("classA", errors.ErrInvalid, "When ClassA and ClassB are the same, specific different hazardous materials must be specified")
		}
	}
