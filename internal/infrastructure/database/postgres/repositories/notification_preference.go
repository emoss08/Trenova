...
func NewNotificationPreferenceRepository(p NotificationPreferenceRepositoryParams) repositories.NotificationPreferenceRepository {
	log := p.Logger.With().
...
func (npr *notificationPreferenceRepository) Create(
	...
	if _, err = dba.NewInsert().Model(pref).Returning("*").Exec(ctx); err != nil {
		log.Error().Err(err).Interface("preference", pref).Msg("failed to insert notification preference")
...
func (npr *notificationPreferenceRepository) Update(
	...
	if err != nil {
		log.Error().Err(err).Interface("preference", pref).Msg("failed to update notification preference")
