...
CREATE UNIQUE INDEX "idx_service_types_code" ON "service_types" (
    lower("code"),
    "organization_id"
);
...
ALTER TABLE "shipments"
ADD CONSTRAINT "fk_shipments_service_type" FOREIGN KEY (
    "service_type_id",
    "business_unit_id",
    "organization_id"
...
--bun:split
CREATE
OR REPLACE FUNCTION service_types_search_vector_update() RETURNS TRIGGER AS $$ BEGIN NEW.search_vector := setweight(
...
--bun:split
CREATE TRIGGER service_types_search_vector_trigger BEFORE
INSERT
    OR
UPDATE
    ON service_types FOR EACH ROW EXECUTE FUNCTION service_types_search_vector_update();
...
--bun:split
CREATE INDEX IF NOT EXISTS idx_service_types_trgm_code_description ON service_types USING gin (
