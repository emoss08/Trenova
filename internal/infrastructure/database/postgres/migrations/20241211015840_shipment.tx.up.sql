...
CREATE INDEX IF NOT EXISTS "idx_shipments_business_unit" ON "shipments" (
    "business_unit_id",
    "organization_id"
);
...
--bun:split
CREATE INDEX IF NOT EXISTS idx_shipments_dates_brin ON shipments USING BRIN (
    actual_ship_date,
...
--bun:split
CREATE
OR REPLACE FUNCTION shipments_search_vector_update() RETURNS TRIGGER AS $$ BEGIN NEW.search_vector := setweight(
    to_tsvector('simple', COALESCE(NEW.pro_number, '')),
    'A'
) || setweight(
    to_tsvector('simple', COALESCE(NEW.bol, '')),
    'A'
) || setweight(
    to_tsvector(
        'english',
    ...
    to_tsvector(
        'english',
        COALESCE(CAST(NEW.rating_method AS text), '')
    ),
    'C'
);
...
--bun:split
CREATE TRIGGER shipments_search_vector_trigger BEFORE
INSERT
    OR
UPDATE
    ON shipments FOR EACH ROW EXECUTE FUNCTION shipments_search_vector_update();
...
--bun:split
ALTER TABLE shipments ALTER COLUMN status SET STATISTICS 1000;
...
--bun:split
CREATE INDEX IF NOT EXISTS idx_shipments_trgm_pro_bol ON shipments USING gin (
    (pro_number || ' ' || bol) gin_trgm_ops
);