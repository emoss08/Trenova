...
CREATE TABLE IF NOT EXISTS "equipment_types" (
    ...
    -- Constraints
    CONSTRAINT "pk_equipment_types" PRIMARY KEY (
        "id",
        "business_unit_id",
        "organization_id"
    ),
...
CREATE UNIQUE INDEX "idx_equipment_types_code" ON "equipment_types" (
    lower("code"),
    "organization_id"
);
...
--bun:split
ALTER TABLE "shipments" ADD COLUMN "tractor_type_id" varchar(100);

ALTER TABLE "shipments"
ADD CONSTRAINT "fk_shipments_tractor_type" FOREIGN KEY (
    "tractor_type_id",
    "business_unit_id",
    "organization_id"
...
--bun:split
ALTER TABLE "shipments" ADD COLUMN "trailer_type_id" varchar(100);

ALTER TABLE "shipments"
ADD CONSTRAINT "fk_shipments_trailer_type" FOREIGN KEY (
    "trailer_type_id",
    "business_unit_id",
    "organization_id"
) REFERENCES "equipment_types" (
...
--bun:split
CREATE
OR REPLACE FUNCTION equipment_types_search_vector_update() RETURNS TRIGGER AS $$ BEGIN NEW.search_vector := setweight(
    ...
    to_tsvector('english', COALESCE(CAST(NEW.class AS text), '')),
    'C'
);

-- Auto-update timestamps
NEW.updated_at := EXTRACT(
    EPOCH
    FROM
        CURRENT_TIMESTAMP
) :: bigint;
...
--bun:split
CREATE TRIGGER equipment_types_search_vector_trigger BEFORE
INSERT
    OR
UPDATE
    ON equipment_types FOR EACH ROW EXECUTE FUNCTION equipment_types_search_vector_update();
...
--bun:split
CREATE INDEX IF NOT EXISTS idx_equipment_types_trgm_code_description ON equipment_types USING gin (
    (code || ' ' || description) gin_trgm_ops
);