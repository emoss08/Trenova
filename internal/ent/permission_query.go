...
func (pq *PermissionQuery) QueryBusinessUnit() *BusinessUnitQuery {
	query := (&BusinessUnitClient{config: pq.config}).Query()
...
func (pq *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withBusinessUnit: pq.withBusinessUnit.Clone(),
...
func (pq *PermissionQuery) WithBusinessUnit(opts ...func(*BusinessUnitQuery)) *PermissionQuery {
	query := (&BusinessUnitClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withBusinessUnit = query
	return pq
}
...
//		GroupBy(permission.FieldBusinessUnitID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Permission.Query().
//		Select(permission.FieldBusinessUnitID).
//		Scan(ctx, &v)
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withBusinessUnit; query != nil {
		if err := pq.loadBusinessUnit(ctx, query, nodes, nil,
			func(n *Permission, e *BusinessUnit) { n.Edges.BusinessUnit = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PermissionQuery) loadBusinessUnit(ctx context.Context, query *BusinessUnitQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *BusinessUnit)) error {
...
func (pq *PermissionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withBusinessUnit != nil {
			_spec.Node.AddColumnOnce(permission.FieldBusinessUnitID)
		}
		if pq.withOrganization != nil {
			_spec.Node.AddColumnOnce(permission.FieldOrganizationID)
		}
