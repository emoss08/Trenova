...
func (lq *LocationQuery) QueryBusinessUnit() *BusinessUnitQuery {
	query := (&BusinessUnitClient{config: lq.config}).Query()
...
func (lq *LocationQuery) QueryOrganization() *OrganizationQuery {
...
// QueryLocationCategory chains the current query on the "location_category" edge.
func (lq *LocationQuery) QueryLocationCategory() *LocationCategoryQuery {
	query := (&LocationCategoryClient{config: lq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (lq *LocationQuery) QueryState() *UsStateQuery {
	query := (&UsStateClient{config: lq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (lq *LocationQuery) QueryComments() *LocationCommentQuery {
...
func (lq *LocationQuery) QueryContacts() *LocationContactQuery {
...
func (lq *LocationQuery) WithBusinessUnit(opts ...func(*BusinessUnitQuery)) *LocationQuery {
	query := (&BusinessUnitClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withBusinessUnit = query
	return lq
}

// WithOrganization tells the query-builder to eager-load the nodes that are connected to
// the "organization" edge. The optional arguments are used to configure the query builder of the edge.
func (lq *LocationQuery) WithOrganization(opts ...func(*OrganizationQuery)) *LocationQuery {
	query := (&OrganizationClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithLocationCategory tells the query-builder to eager-load the nodes that are connected to
// the "location_category" edge. The optional arguments are used to configure the query builder of the edge.
func (lq *LocationQuery) WithLocationCategory(opts ...func(*LocationCategoryQuery)) *LocationQuery {
	query := (&LocationCategoryClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withLocationCategory = query
	return lq
}
...
func (lq *LocationQuery) WithState(opts ...func(*UsStateQuery)) *LocationQuery {
	query := (&UsStateClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (lq *LocationQuery) WithComments(opts ...func(*LocationCommentQuery)) *LocationQuery {
	query := (&LocationCommentClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (lq *LocationQuery) WithContacts(opts ...func(*LocationContactQuery)) *LocationQuery {
	query := (&LocationContactClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(location.FieldBusinessUnitID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Location.Query().
//		Select(location.FieldBusinessUnitID).
//		Scan(ctx, &v)
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withBusinessUnit; query != nil {
		if err := lq.loadBusinessUnit(ctx, query, nodes, nil,
			func(n *Location, e *BusinessUnit) { n.Edges.BusinessUnit = e }); err != nil {
			return nil, err
		}
	}
	if query := lq.withOrganization; query != nil {
		if err := lq.loadOrganization(ctx, query, nodes, nil,
			func(n *Location, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	if query := lq.withLocationCategory; query != nil {
		if err := lq.loadLocationCategory(ctx, query, nodes, nil,
			func(n *Location, e *LocationCategory) { n.Edges.LocationCategory = e }); err != nil {
			return nil, err
		}
	}
	if query := lq.withState; query != nil {
		if err := lq.loadState(ctx, query, nodes, nil,
			func(n *Location, e *UsState) { n.Edges.State = e }); err != nil {
			return nil, err
		}
	}
	if query := lq.withComments; query != nil {
		if err := lq.loadComments(ctx, query, nodes,
			func(n *Location) { n.Edges.Comments = []*LocationComment{} },
			func(n *Location, e *LocationComment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
...
func (lq *LocationQuery) loadBusinessUnit(ctx context.Context, query *BusinessUnitQuery, nodes []*Location, init func(*Location), assign func(*Location, *BusinessUnit)) error {
...
func (lq *LocationQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Location, init func(*Location), assign func(*Location, *Organization)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Location)
	for i := range nodes {
...
func (lq *LocationQuery) loadLocationCategory(ctx context.Context, query *LocationCategoryQuery, nodes []*Location, init func(*Location), assign func(*Location, *LocationCategory)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Location)
	for i := range nodes {
		if nodes[i].LocationCategoryID == nil {
			continue
		}
		fk := *nodes[i].LocationCategoryID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(locationcategory.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "location_category_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (lq *LocationQuery) loadState(ctx context.Context, query *UsStateQuery, nodes []*Location, init func(*Location), assign func(*Location, *UsState)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Location)
	for i := range nodes {
		fk := nodes[i].StateID
...
func (lq *LocationQuery) loadComments(ctx context.Context, query *LocationCommentQuery, nodes []*Location, init func(*Location), assign func(*Location, *LocationComment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(locationcomment.FieldLocationID)
	}
	query.Where(predicate.LocationComment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.CommentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (lq *LocationQuery) loadContacts(ctx context.Context, query *LocationContactQuery, nodes []*Location, init func(*Location), assign func(*Location, *LocationContact)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.LocationContact(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.ContactsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (lq *LocationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := lq.ctx.Fields; len(fields) > 0 {
		...
		if lq.withBusinessUnit != nil {
			_spec.Node.AddColumnOnce(location.FieldBusinessUnitID)
		}
		if lq.withOrganization != nil {
			_spec.Node.AddColumnOnce(location.FieldOrganizationID)
		}
		if lq.withLocationCategory != nil {
			_spec.Node.AddColumnOnce(location.FieldLocationCategoryID)
		}
		if lq.withState != nil {
			_spec.Node.AddColumnOnce(location.FieldStateID)
		}
	}
...
// Modify adds a query modifier for attaching custom logic to queries.
func (ls *LocationSelect) Modify(modifiers ...func(s *sql.Selector)) *LocationSelect {
