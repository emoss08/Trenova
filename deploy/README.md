...
- Errors are intermittent and retrying the same request succeeds

**Solutions:**

1. Ensure VITE_API_URL matches the actual domain
2. Verify `server.cors.allowedOrigins` and `security.csrf.trustedOrigins` include the browser origin
3. Verify Caddy or Cloudflare Tunnel is routing to the healthy API container
4. Check browser network tab for actual request URLs and HTTP status codes

For Cloudflare Tunnel deployments, test the public API from a shell:

```bash
curl -si \
  -H 'Origin: https://cloud.trenova.app' \
  https://api.trenova.app/api/v1/system/version

curl -si -X OPTIONS \
  -H 'Origin: https://cloud.trenova.app' \
  -H 'Access-Control-Request-Method: GET' \
  -H 'Access-Control-Request-Headers: authorization,content-type,x-csrf-token' \
  https://api.trenova.app/api/v1/shipments/
```

If those responses include `Access-Control-Allow-Origin` but the browser intermittently reports it missing, the failing response is likely being generated by Cloudflare or the tunnel before it reaches the Go API. Check `cloudflared` and `tms-api` logs around the failing `cf-ray` timestamp and confirm the API container is not restarting or failing health checks.
