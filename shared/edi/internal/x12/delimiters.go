...
// extractISAElement returns the Nth ISA element contents (1-based after the tag), without delimiters.
func extractISAElement(data []byte, elem byte, n int) ([]byte, bool) {
    // Expect data starts with 'ISA' + elem
    i := 4
    start := i
    count := 0
    for i < len(data) {
        ...
        // terminate if we hit a segment end by heuristic (common '~' or '\n')
        if data[i] == '~' || data[i] == '\n' || data[i] == '\r' {
