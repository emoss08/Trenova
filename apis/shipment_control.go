...
func GetShipmentControl(db *gorm.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		tools.ResponseWithJSON(w, http.StatusOK, sc)
	}
}

func UpdateShipmentControl(db *gorm.DB, validator *tools.Validator) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		orgID, ok := r.Context().Value(middleware.ContextKeyOrgID).(uuid.UUID)
		...
		if err := tools.ParseBodyAndValidate(validator, w, r, &sc); err != nil {
			return
		}
