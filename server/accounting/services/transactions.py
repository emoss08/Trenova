...
@transaction.atomic
def create_transaction_from_shipment(*, shipment: Shipment) -> None:
    ...
    f_transaction = _create_financial_transaction(
        shipment=shipment, revenue_account=revenue_account
    )

    if not _has_sufficient_data(shipment=shipment, control=control):
        missing_data = _identify_missing_data(shipment=shipment)
        _log_to_reconciliation_queue(
            shipment=shipment,
...
def _create_financial_transaction(
    *, shipment: Shipment, revenue_account: models.GeneralLedgerAccount
...
def _has_sufficient_data(
    *, shipment: Shipment, control: models.AccountingControl
) -> bool:
...
def _identify_missing_data(*, shipment: Shipment) -> str:
...
def _match_journal_entry_criteria(*, shipment: Shipment, criteria: str) -> bool:
...
def _handle_reconciliation(
    *,
    ...
    Returns:
        None: This function does not return anything.
    """
    requires_reconciliation, reason = _shipment_requires_reconciliation(
        shipment=shipment, f_transaction=f_transaction, control=control
    )

    if requires_reconciliation and control.enable_reconciliation_notifications:
        _notify_reconciliation_required(
        ...
        _log_reconciliation_event(shipment=shipment)


def _notify_reconciliation_required(
    *, shipment: Shipment, reason: str, control: models.AccountingControl
) -> None:
    ...
    try:
        ...
        logger.error(f"Error sending reconciliation notification email: {email_error}")


def _log_reconciliation_event(*, shipment: Shipment) -> None:
...
def _shipment_requires_reconciliation(
    *,
    ...
    Returns:
        tuple[bool, str]: A Pair of boolean indicating if reconciliation is required and a string
    ...
    if not _match_journal_entry_criteria(shipment=shipment, criteria=criteria):
        return True, (
